...
- Dynamiska moduler som skapas av `New-Module` cmdleten kan namnges `Name` med hjälp av parametern för cmdleten.

- Moduler som importeras från Assembly `"dynamic_code_module_" + assembly.GetName()`  **`Import-Module` -** objekt med kommandot-Assembly namnges enligt följande syntax:.
...
[Export-ModuleMember-](/powershell/module/Microsoft.PowerShell.Core/Export-ModuleMember) cmdlet denna cmdlet anger de Modulnamn (till exempel cmdlets, functions, variabler och alias) som exporteras från en skriptfil (. psm1) eller från en dynamisk modul som skapats med hjälp `New-Module` av cmdleten.
