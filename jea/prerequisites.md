...
Just Enough Administration är en funktion som ingår i PowerShell 5.0 och senare. Den här artikeln beskrivs de krav som måste uppfyllas för att börja använda JEA.
...
JEA är tillgängliga i PowerShell 5.0 och senare. Full funktionalitet får rekommenderar vi att du installerar den senaste versionen av PowerShell som är tillgängliga för ditt system. I följande tabell beskrivs JEAS tillgänglighet på Windows Server:
...
- <sup>1</sup> JEA kan inte konfigureras för att använda gruppen hanterade tjänstkonton på Windows Server 2008 R2 eller Windows 7. Virtuella konton och andra funktioner för JEA *är* stöds.

- <sup>2</sup> följande JEA-funktioner stöds inte på Windows 10 version 1511 och 1603:

  - Körs som ett-grupphanterat tjänstkonto
  - Regler för villkorlig åtkomst i sessionskonfigurationer
  - Användare-enhet
...
Om du kör en äldre version av PowerShell, kan du behöva uppdatera datorn med den senaste uppdateringen av Windows Management Framework (WMF). Mer information finns i den [WMF dokumentation](/powershell/wmf/overview).

Vi rekommenderar att du testar din arbetsbelastning kompatibilitet med WMF innan du uppgraderar alla dina servrar.
...
PowerShell-fjärrkommunikation utgör grunden som JEA har skapats. Det är nödvändigt att säkerställa att PowerShell-fjärrkommunikation är aktiverad och korrekt skyddade innan du kan använda JEA. Mer information finns i [WinRM Security](/powershell/scripting/learn/remoting/winrmsecurity).

PowerShell-fjärrkommunikation är aktiverat som standard på Windows Server 2012, 2012 R2 och 2016. Du kan aktivera PowerShell-fjärrkommunikation genom att köra följande kommando i ett upphöjt PowerShell-fönster.
...
Följande steg aktiverar loggning för alla PowerShell-åtgärder på datorn. Loggning av PowerShell-modulen krävs inte för JEA, men det rekommenderas du aktiverar loggning för att se till att köra kommandon användare är inloggade på en central plats.
...
6. Typ `*` i popup-fönstret för att logga kommandon från alla moduler.
7. Klicka på **OK** att ställa in principen
8. Dubbelklicka på **aktivera PowerShell-skriptblock loggning**
...
[PowerShell ♥ blå teamet](https://devblogs.microsoft.com/powershell/powershell-the-blue-team/)
