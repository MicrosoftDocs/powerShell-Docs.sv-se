...
---
# <a name="resource-dependencies-using-dependson"></a><span data-ttu-id="686a3-103">Resursberoenden med DependsOn</span><span class="sxs-lookup"><span data-stu-id="686a3-103">Resource dependencies using DependsOn</span></span>

<span data-ttu-id="686a3-104">När du skriver [konfigurationer](configurations.md), du lägger till [Resource block](../resources/resources.md) konfigurera aspekter av ett mål noden.</span><span class="sxs-lookup"><span data-stu-id="686a3-104">When you write [Configurations](configurations.md), you add [Resource blocks](../resources/resources.md) to configure aspects of a target Node.</span></span> <span data-ttu-id="686a3-105">När du fortsätter att lägga till resursen block kan dina konfigurationer växa ganska stora och komplicerade att hantera.</span><span class="sxs-lookup"><span data-stu-id="686a3-105">As you continue to add Resource blocks, your Configurations can grow quite large and cumbersome to manage.</span></span> <span data-ttu-id="686a3-106">En utmaning är tillämpad ordning av resource-block.</span><span class="sxs-lookup"><span data-stu-id="686a3-106">One such challenge is the applied order of your resource blocks.</span></span> <span data-ttu-id="686a3-107">Vanligtvis tillämpas resurser i den ordning som de definieras i konfigurationen.</span><span class="sxs-lookup"><span data-stu-id="686a3-107">Typically resources are applied in the order they are defined within the Configuration.</span></span> <span data-ttu-id="686a3-108">När konfigurationen växer större och mer komplexa, kan du använda den `DependsOn` för att ändra tillämpade ordningen på dina resurser genom att ange som en resurs är beroende av en annan resurs.</span><span class="sxs-lookup"><span data-stu-id="686a3-108">As your Configuration grows larger and more complex, you can use the `DependsOn` key to change the applied order of your resources by specifying that a resource depends on another resource.</span></span>

<span data-ttu-id="686a3-109">Den `DependsOn` nyckeln kan användas i valfri resurs-block.</span><span class="sxs-lookup"><span data-stu-id="686a3-109">The `DependsOn` key can be used in any Resource block.</span></span> <span data-ttu-id="686a3-110">Den har definierats med samma nyckel/värde-mekanism som andra Resource-nycklar.</span><span class="sxs-lookup"><span data-stu-id="686a3-110">It is defined with the same key/value mechanism as other Resource keys.</span></span> <span data-ttu-id="686a3-111">Den `DependsOn` nyckel förväntar sig en matris med strängar med följande syntax.</span><span class="sxs-lookup"><span data-stu-id="686a3-111">The `DependsOn` key expects an array of strings with the following syntax.</span></span>

```
DependsOn = '[<Resource Type>]<Resource Name>', '[<Resource Type>]<Resource Name'
```

<span data-ttu-id="686a3-112">I följande exempel konfigureras en brandväggsregel efter aktivering och konfigurering av den offentliga profilen.</span><span class="sxs-lookup"><span data-stu-id="686a3-112">The following example configures a firewall rule after enabling and configuring the public profile.</span></span>
...
<span data-ttu-id="686a3-113">När du tillämpar konfigurationen konfigureras brandväggsprofilen alltid först oavsett vilken ordning resurs-blocken har definierats.</span><span class="sxs-lookup"><span data-stu-id="686a3-113">When you apply the Configuration, the firewall profile will always be configured first regardless of which order the Resource blocks are defined.</span></span> <span data-ttu-id="686a3-114">Om du använder konfigurationen, bör du Observera din Målnoder befintliga konfiguration så att du kan återställa om du vill.</span><span class="sxs-lookup"><span data-stu-id="686a3-114">If you apply the Configuration, be sure to note your target Nodes existing Configuration so you can revert if desired.</span></span>
...
<span data-ttu-id="686a3-115">Detta säkerställer också att om den **FirewallProfile** resursen misslyckas av någon anledning, den **brandväggen** kommer inte att köra block även om det först har definierats.</span><span class="sxs-lookup"><span data-stu-id="686a3-115">This also ensures that if the **FirewallProfile** resource fails for any reason, the **Firewall** block will not execute even though it was defined first.</span></span> <span data-ttu-id="686a3-116">Den `DependsOn` nyckel ger större flexibilitet i gruppera resource block och att säkerställa att beroenden är löst innan en resurs kan köras.</span><span class="sxs-lookup"><span data-stu-id="686a3-116">The `DependsOn` key allows more flexibility in grouping resource blocks and ensuring that dependencies are resolved before a Resource executes.</span></span>

<span data-ttu-id="686a3-117">I mer avancerade konfigurationer kan du också använda [över flera noder beroende](crossNodeDependencies.md) att tillåta ännu mer detaljerad kontroll (T.ex, att se till att en domänkontrollant är konfigurerad innan du ansluter en klient till domänen).</span><span class="sxs-lookup"><span data-stu-id="686a3-117">In more advanced Configurations, you can also use [Cross Node Dependency](crossNodeDependencies.md) to allow even more granular control (For example, ensuring a domain controller is configured before joining a client to the domain).</span></span>

## <a name="cleaning-up"></a><span data-ttu-id="686a3-118">Rensa</span><span class="sxs-lookup"><span data-stu-id="686a3-118">Cleaning Up</span></span>

<span data-ttu-id="686a3-119">Om du har tillämpat konfigurationen ovan kan du återställa nycklar för att ångra eventuella ändringar.</span><span class="sxs-lookup"><span data-stu-id="686a3-119">If you applied the Configuration above, you can reverse keys to undo any changes.</span></span> <span data-ttu-id="686a3-120">I exemplet ovan, ställa in den **aktiverad** nyckeln till false inaktiverar brandväggsregel och profil.</span><span class="sxs-lookup"><span data-stu-id="686a3-120">In the above example, setting the **Enabled** key to false will disable the firewall rule and profile.</span></span> <span data-ttu-id="686a3-121">Du bör ändra exemplet som behövs för att matcha dina målnoden tidigare konfigurerade tillstånd.</span><span class="sxs-lookup"><span data-stu-id="686a3-121">You should modify the example as needed to match your target Node's previous configured state.</span></span>
...
## <a name="see-also"></a><span data-ttu-id="686a3-122">Se även</span><span class="sxs-lookup"><span data-stu-id="686a3-122">See also</span></span>

- [<span data-ttu-id="686a3-123">Använd beroenden mellan noder</span><span class="sxs-lookup"><span data-stu-id="686a3-123">Use Cross Node Dependencies</span></span>](./crossNodeDependencies.md)
