...
I-skript blocket hämtar det första kommandot alla tjänster som körs med hjälp av `Where-Object` cmdleten. Tjänsterna är skickas till `ForEach-Object` cmdleten. `ForEach-Object`Cmdleten skapar ett nytt [`[System.Management.Automation.CompletionResult]`](/dotnet/api/system.management.automation.completionresult) objekt och fyller det med värdena för den aktuella tjänsten (som representeras av pipeline-variabeln `$_` ).
