...
När du använder `powershell.exe` med **fil** parametern `.ps1` bör (skript filen) innehålla instruktioner för hantering av eventuella fel eller undantag som inträffar när skriptet körs. Du bör endast använda `exit` instruktionen för att ange skriptets efter körnings status.

Alla tal mellan `[int]::MinValue` och `[int]::MaxValue` är tillåtna.
