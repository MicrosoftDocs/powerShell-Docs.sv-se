...
Dessutom kan insamlade grupper refereras till i \<substitute\> strängen.
Detta görs med hjälp av `$` specialtecknet före grupp-ID: n.

Två av metoderna för att referera till infångade grupper är efter **nummer** och efter **namn**

- Som **antal** -
   insamlade grupper numreras från vänster till höger.
...
- Efter **namn** -
   fångar grupper kan också refereras efter namn.
...
> [!WARNING]
> Eftersom det `$` används i sträng expansion måste du använda litterala strängar med ersättning eller Escape- `$` tecken.
>
> ```powershell
