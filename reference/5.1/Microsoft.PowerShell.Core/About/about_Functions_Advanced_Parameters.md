...
Det finns ett känt problem med att använda samlingar med **ValueFromRemainingArguments** där den överförda samlingen behandlas som ett enda element.

Följande exempel visar det här kända problemet. Den **återstående** parametern ska innehålla **en** vid **index 0** och **två** vid **index 1**.
I stället kombineras båda elementen i en enda entitet.
...
> [!NOTE]
> Det här problemet har lösts i PowerShell 6,2.
...
Attributet **ValidateRange** anger ett numeriskt intervall för varje parameter eller variabel värde. PowerShell genererar ett fel om ett värde ligger utanför intervallet. I följande exempel måste värdet **för parametern** Parameters vara mellan noll och tio.
