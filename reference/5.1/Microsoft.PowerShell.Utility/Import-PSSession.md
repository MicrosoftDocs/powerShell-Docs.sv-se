...
Fjärråtgärder hanteras dock helt av Windows PowerShell.
...
Med den här parametern kan du köra kommandot som ett bakgrunds jobb i Windows PowerShell.
Mer information finns i artikeln om jobb.

När du använder **import-PSSession** lägger Windows PowerShell till de importerade kommandona till en tillfällig modul som bara finns i din session och returnerar ett objekt som representerar modulen.
Använd Export-PSSession-cmdleten om du vill skapa en beständig modul som du kan använda i framtida sessioner.

Cmdlet **: en import-PSSession** använder funktionen för implicit fjärr kommunikation i Windows PowerShell.
...
Normalt läggs modulen till i alla sessioner med kommandot **import-module** i en Windows PowerShell-profil, men profilerna körs inte i PSSessions.
...
Eftersom Functions har företräde framför cmdlets, kör Windows PowerShell den importerade **Get-date-** funktionen som returnerar ett Juliansk-datum.
...
Det fjärde kommandot hämtar namnet på Windows PowerShell-snapin-modulen som lade till den ursprungliga cmdleten **Get-date** i den aktuella sessionen.
...
Men när du skickar ett modul-objekt använder Windows PowerShell metoden **toString** i objektet module, som returnerar modulnamnet.
...
- Applicering.
Alla filer utom Windows-PowerShell filer i Sök vägarna som anges i miljövariabeln PATH ($env:p ökväg) i fjärrsessionen, inklusive. txt-,. exe-och. dll-filer.
- Kommandon.
...
### – Modul
Anger och en matris med kommandon i Windows PowerShell snapin-moduler och moduler.
...
* **Import-PSSession** förlitar sig på infrastrukturen för fjärr kommunikation i Windows PowerShell. Om du vill använda denna cmdlet måste datorn konfigureras för WS-Management fjärr kommunikation. Mer information finns i about_Remote och about_Remote_Requirements.
* **Import-PSSession** importerar inte variabler eller Windows PowerShell-providrar.
