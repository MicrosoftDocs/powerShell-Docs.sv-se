...
`Export-FormatData`Cmdleten skapar filer för Windows PowerShell-formatering (format.ps1XML) från formateringselement i den aktuella sessionen. Det tar **ExtendedTypeDefinition** -objekt som `Get-FormatData` returnerar och sparar dem i en fil i XML-format.

Windows PowerShell använder datan i att formatera filer (format.ps1XML) för att generera standard visningen av Microsoft .NET Ramverks objekt i sessionen. Du kan visa och redigera formateringsattribut och använda Update-FormatData cmdlet för att lägga till formaterings data i en session.

Mer information om hur du formaterar filer i Windows PowerShell finns i [about_Format.ps1XML](../Microsoft.PowerShell.Core/About/about_Format.ps1xml.md).
