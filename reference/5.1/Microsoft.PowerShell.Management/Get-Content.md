...
Det första kommandot använder **encoding** -parametern för att hämta data strömmen av byte från filen.
...
När du läser från och skriver till binära filer använder du värdet **byte** för den dynamiska parametern **encoding** och värdet 0 för parametern **ReadCount** . Ett **ReadCount** -värde på 0 läser hela filen i en enda Läs åtgärd och konverterar den till ett enda objekt (PSObject). Standard värde för **ReadCount** , 1, läser en byte i varje Läs åtgärd och konverterar varje byte till ett separat objekt, vilket orsakar fel när du använder `Set-Content` cmdleten för att skriva byte till en fil.
