...
<span data-ttu-id="13a70-112">I följande tabell visas inställningarna för variabler och standardvärden.</span><span class="sxs-lookup"><span data-stu-id="13a70-112">The following table lists the preference variables and their default values.</span></span>
...
## <a name="working-remotely"></a><span data-ttu-id="13a70-153">Arbeta fjärran slutet</span><span class="sxs-lookup"><span data-stu-id="13a70-153">Working remotely</span></span>
...
<span data-ttu-id="13a70-167">Giltiga värden för `$ConfirmPreference` :</span><span class="sxs-lookup"><span data-stu-id="13a70-167">Valid values of `$ConfirmPreference`:</span></span>
...
#### <a name="examples"></a><span data-ttu-id="13a70-204">Exempel</span><span class="sxs-lookup"><span data-stu-id="13a70-204">Examples</span></span>
...
<span data-ttu-id="13a70-281">Det här exemplet visar hur samma fel visas när värdet för `$ErrorView` ändras till **CategoryView**.</span><span class="sxs-lookup"><span data-stu-id="13a70-281">This example shows how the same error appears when the value of `$ErrorView` is changed to **CategoryView**.</span></span>
...
### <a name="formatenumerationlimit"></a><span data-ttu-id="13a70-286">\$FormatEnumerationLimit</span><span class="sxs-lookup"><span data-stu-id="13a70-286">\$FormatEnumerationLimit</span></span>
...
<span data-ttu-id="13a70-368">Det första kommandot hittar värdet för `$OutputEncoding` .</span><span class="sxs-lookup"><span data-stu-id="13a70-368">The first command finds the value of `$OutputEncoding`.</span></span> <span data-ttu-id="13a70-369">Eftersom värdet är ett encoding-objekt visas endast dess **EncodingName** -egenskap.</span><span class="sxs-lookup"><span data-stu-id="13a70-369">Because the value is an encoding object, display only its **EncodingName** property.</span></span>
...
### <a name="progresspreference"></a><span data-ttu-id="13a70-376">\$ProgressPreference</span><span class="sxs-lookup"><span data-stu-id="13a70-376">\$ProgressPreference</span></span>
...
<span data-ttu-id="13a70-391">Anger standardvärden för parametrarna för cmdlets och Advanced functions.</span><span class="sxs-lookup"><span data-stu-id="13a70-391">Specifies default values for the parameters of cmdlets and advanced functions.</span></span>
...
<span data-ttu-id="13a70-408">Mer information om automatisk import av moduler finns i [about_Modules](about_Modules.md).</span><span class="sxs-lookup"><span data-stu-id="13a70-408">For more information about automatic importing of modules, see [about_Modules](about_Modules.md).</span></span>
...
<span data-ttu-id="13a70-428">Du kan när som helst ändra värdet för den här variabeln.</span><span class="sxs-lookup"><span data-stu-id="13a70-428">You can change the value of this variable at any time.</span></span> <span data-ttu-id="13a70-429">När du gör det måste du komma ihåg att den konfiguration av sessionen som du väljer måste finnas på fjärrdatorn.</span><span class="sxs-lookup"><span data-stu-id="13a70-429">When you do, remember that the session configuration that you select must exist on the remote computer.</span></span> <span data-ttu-id="13a70-430">Om den inte gör det Miss lyckas kommandot för att skapa en session som använder konfigurationen av sessionen.</span><span class="sxs-lookup"><span data-stu-id="13a70-430">If it doesn't, the command to create a session that uses the session configuration fails.</span></span>

<span data-ttu-id="13a70-431">Den här preferens variabeln avgör inte vilka konfigurationer för lokal session som används när fjärranslutna användare skapar en session som ansluter till den här datorn.</span><span class="sxs-lookup"><span data-stu-id="13a70-431">This preference variable doesn't determine which local session configurations are used when remote users create a session that connects to this computer.</span></span>
<span data-ttu-id="13a70-432">Du kan dock använda behörigheterna för de lokala session-konfigurationerna för att avgöra vilka användare som kan använda dem.</span><span class="sxs-lookup"><span data-stu-id="13a70-432">However, you can use the permissions for the local session configurations to determine which users may use them.</span></span>

### <a name="pssessionoption"></a><span data-ttu-id="13a70-433">\$PSSessionOption</span><span class="sxs-lookup"><span data-stu-id="13a70-433">\$PSSessionOption</span></span>

<span data-ttu-id="13a70-434">Fastställer standardvärden för avancerade användar alternativ i en fjärrsession.</span><span class="sxs-lookup"><span data-stu-id="13a70-434">Establishes the default values for advanced user options in a remote session.</span></span>
<span data-ttu-id="13a70-435">Dessa alternativ inställningar åsidosätter systemets standardvärden för session-alternativ.</span><span class="sxs-lookup"><span data-stu-id="13a70-435">These option preferences override the system default values for session options.</span></span>

<span data-ttu-id="13a70-436">`$PSSessionOption`Variabeln innehåller ett **PSSessionOption** -objekt.</span><span class="sxs-lookup"><span data-stu-id="13a70-436">The `$PSSessionOption` variable contains a **PSSessionOption** object.</span></span> <span data-ttu-id="13a70-437">Mer information finns i [system. Management. Automation. Remoting. PSSessionOption](/dotnet/api/system.management.automation.remoting.pssessionoption).</span><span class="sxs-lookup"><span data-stu-id="13a70-437">For more information, see [System.Management.Automation.Remoting.PSSessionOption](/dotnet/api/system.management.automation.remoting.pssessionoption).</span></span>
<span data-ttu-id="13a70-438">Varje egenskap för objektet representerar ett session-alternativ.</span><span class="sxs-lookup"><span data-stu-id="13a70-438">Each property of the object represents a session option.</span></span> <span data-ttu-id="13a70-439">Egenskapen **nocompression** aktiverar till exempel data komprimering under sessionen.</span><span class="sxs-lookup"><span data-stu-id="13a70-439">For example, the **NoCompression** property turns of data compression during the session.</span></span>

<span data-ttu-id="13a70-440">Som standard `$PSSessionOption` innehåller variabeln ett **PSSessionOption** -objekt med standardvärdena för alla alternativ, som du ser nedan.</span><span class="sxs-lookup"><span data-stu-id="13a70-440">By default, the `$PSSessionOption` variable contains a **PSSessionOption** object with the default values for all options, as shown below.</span></span>
...
<span data-ttu-id="13a70-441">Beskrivningar av dessa alternativ och mer information finns i [New-PSSessionOption](xref:Microsoft.PowerShell.Core.New-PSSessionOption).</span><span class="sxs-lookup"><span data-stu-id="13a70-441">For descriptions of these options and more information, see [New-PSSessionOption](xref:Microsoft.PowerShell.Core.New-PSSessionOption).</span></span> <span data-ttu-id="13a70-442">Mer information om fjärrkommandon och sessioner finns i [about_Remote](about_Remote.md) och [about_PSSessions](about_PSSessions.md).</span><span class="sxs-lookup"><span data-stu-id="13a70-442">For more information about remote commands and sessions, see [about_Remote](about_Remote.md) and [about_PSSessions](about_PSSessions.md).</span></span>

<span data-ttu-id="13a70-443">Om du vill ändra värdet på `$PSSessionOption` variabeln Preference använder du `New-PSSessionOption` cmdleten för att skapa ett **PSSessionOption** -objekt med de alternativ värden som du föredrar.</span><span class="sxs-lookup"><span data-stu-id="13a70-443">To change the value of the `$PSSessionOption` preference variable, use the `New-PSSessionOption` cmdlet to create a **PSSessionOption** object with the option values you prefer.</span></span> <span data-ttu-id="13a70-444">Spara utdata i en variabel som kallas `$PSSessionOption` .</span><span class="sxs-lookup"><span data-stu-id="13a70-444">Save the output in a variable called `$PSSessionOption`.</span></span>

```powershell
$PSSessionOption = New-PSSessionOption -NoCompression
```

<span data-ttu-id="13a70-445">Om du vill använda `$PSSessionOption` variabeln Preference i varje PowerShell-session lägger du till ett `New-PSSessionOption` kommando som skapar `$PSSessionOption` variabeln i din PowerShell-profil.</span><span class="sxs-lookup"><span data-stu-id="13a70-445">To use the `$PSSessionOption` preference variable in every PowerShell session, add a `New-PSSessionOption` command that creates the `$PSSessionOption` variable to your PowerShell profile.</span></span> <span data-ttu-id="13a70-446">Mer information finns i [about_Profiles](about_Profiles.md).</span><span class="sxs-lookup"><span data-stu-id="13a70-446">For more information, see [about_Profiles](about_Profiles.md).</span></span>

<span data-ttu-id="13a70-447">Du kan ange anpassade alternativ för en viss fjärrsession.</span><span class="sxs-lookup"><span data-stu-id="13a70-447">You can set custom options for a particular remote session.</span></span> <span data-ttu-id="13a70-448">De alternativ som du anger prioriteras framför systemets standardinställningar och värdet för Preference- `$PSSessionOption` variabeln.</span><span class="sxs-lookup"><span data-stu-id="13a70-448">The options that you set take precedence over the system defaults and the value of the `$PSSessionOption` preference variable.</span></span>

<span data-ttu-id="13a70-449">Om du vill ange alternativ för anpassade sessioner använder du `New-PSSessionOption` cmdleten för att skapa ett **PSSessionOption** -objekt.</span><span class="sxs-lookup"><span data-stu-id="13a70-449">To set custom session options, use the `New-PSSessionOption` cmdlet to create a **PSSessionOption** object.</span></span> <span data-ttu-id="13a70-450">Använd sedan **PSSessionOption** -objektet som värdet för parametern **SessionOption** i cmdletar som skapar en session, till exempel, `New-PSSession` `Enter-PSSession` och `Invoke-Command` .</span><span class="sxs-lookup"><span data-stu-id="13a70-450">Then, use the **PSSessionOption** object as the value of the **SessionOption** parameter in cmdlets that create a session, such as `New-PSSession`, `Enter-PSSession`, and `Invoke-Command`.</span></span>

### <a name="transcript"></a><span data-ttu-id="13a70-451">$Transcript</span><span class="sxs-lookup"><span data-stu-id="13a70-451">$Transcript</span></span>

<span data-ttu-id="13a70-452">Används av `Start-Transcript` för att ange namn och plats för avskrifts filen.</span><span class="sxs-lookup"><span data-stu-id="13a70-452">Used by `Start-Transcript` to specify the name and location of the transcript file.</span></span> <span data-ttu-id="13a70-453">Om du inte anger något värde för parametern **Path** `Start-Transcript` använder sökvägen i värdet för den `$Transcript` globala variabeln.</span><span class="sxs-lookup"><span data-stu-id="13a70-453">If you do not specify a value for the **Path** parameter, `Start-Transcript` uses the path in the value of the `$Transcript` global variable.</span></span> <span data-ttu-id="13a70-454">Om du inte har skapat den här variabeln `Start-Transcript` lagras avskrifterna i `$Home\My Documents` katalogen som `\PowerShell_transcript.<time-stamp>.txt` filer.</span><span class="sxs-lookup"><span data-stu-id="13a70-454">If you have not created this variable, `Start-Transcript` stores the transcripts in the `$Home\My Documents` directory as `\PowerShell_transcript.<time-stamp>.txt` files.</span></span>

### <a name="verbosepreference"></a><span data-ttu-id="13a70-455">\$VerbosePreference</span><span class="sxs-lookup"><span data-stu-id="13a70-455">\$VerbosePreference</span></span>

<span data-ttu-id="13a70-456">Fastställer hur PowerShell svarar på utförliga meddelanden som genereras av ett skript, en cmdlet eller en provider, till exempel de meddelanden som genereras av [Write-Verbose-](xref:Microsoft.PowerShell.Utility.Write-Verbose) cmdleten.</span><span class="sxs-lookup"><span data-stu-id="13a70-456">Determines how PowerShell responds to verbose messages generated by a script, cmdlet, or provider, such as the messages generated by the [Write-Verbose](xref:Microsoft.PowerShell.Utility.Write-Verbose) cmdlet.</span></span>
<span data-ttu-id="13a70-457">Utförliga meddelanden beskriver de åtgärder som utförs för att köra ett kommando.</span><span class="sxs-lookup"><span data-stu-id="13a70-457">Verbose messages describe the actions performed to execute a command.</span></span>

<span data-ttu-id="13a70-458">Som standard visas inte utförliga meddelanden, men du kan ändra det här beteendet genom att ändra värdet för `$VerbosePreference` .</span><span class="sxs-lookup"><span data-stu-id="13a70-458">By default, verbose messages aren't displayed, but you can change this behavior by changing the value of `$VerbosePreference`.</span></span>

<span data-ttu-id="13a70-459">Du kan använda den **utförliga** gemensamma parametern för en cmdlet för att visa eller dölja utförliga meddelanden för ett speciellt kommando.</span><span class="sxs-lookup"><span data-stu-id="13a70-459">You can use the **Verbose** common parameter of a cmdlet to display or hide the verbose messages for a specific command.</span></span> <span data-ttu-id="13a70-460">Mer information finns i [about_CommonParameters](about_CommonParameters.md).</span><span class="sxs-lookup"><span data-stu-id="13a70-460">For more information, see [about_CommonParameters](about_CommonParameters.md).</span></span>

<span data-ttu-id="13a70-461">Giltiga värden är följande:</span><span class="sxs-lookup"><span data-stu-id="13a70-461">The valid values are as follows:</span></span>

- <span data-ttu-id="13a70-462">**Stop** : visar utförligt meddelande och ett fel meddelande och sedan slutar att köra.</span><span class="sxs-lookup"><span data-stu-id="13a70-462">**Stop** : Displays the verbose message and an error message and then stops executing.</span></span>
- <span data-ttu-id="13a70-463">**Fråga** : visar utförligt meddelande och visar en uppmaning som frågar om du vill fortsätta.</span><span class="sxs-lookup"><span data-stu-id="13a70-463">**Inquire** : Displays the verbose message and then displays a prompt that asks you whether you want to continue.</span></span>
- <span data-ttu-id="13a70-464">**Fortsätt** : visar utförligt meddelande och fortsätter sedan med körningen.</span><span class="sxs-lookup"><span data-stu-id="13a70-464">**Continue** : Displays the verbose message and then continues with execution.</span></span>
- <span data-ttu-id="13a70-465">**SilentlyContinue** : (standard) visar inte utförligt meddelande.</span><span class="sxs-lookup"><span data-stu-id="13a70-465">**SilentlyContinue** : (Default) Doesn't display the verbose message.</span></span> <span data-ttu-id="13a70-466">Fortsätter att köra.</span><span class="sxs-lookup"><span data-stu-id="13a70-466">Continues executing.</span></span>

#### <a name="examples"></a><span data-ttu-id="13a70-467">Exempel</span><span class="sxs-lookup"><span data-stu-id="13a70-467">Examples</span></span>

<span data-ttu-id="13a70-468">I de här exemplen visas effekterna av de olika värdena i `$VerbosePreference` och **verbose** -parametern för att åsidosätta Preference-värdet.</span><span class="sxs-lookup"><span data-stu-id="13a70-468">These examples show the effect of the different values of `$VerbosePreference` and the **Verbose** parameter to override the preference value.</span></span>

<span data-ttu-id="13a70-469">Det här exemplet visar resultatet av **SilentlyContinue** -värdet, som är standardvärdet.</span><span class="sxs-lookup"><span data-stu-id="13a70-469">This example shows the effect of the **SilentlyContinue** value, that is the default.</span></span> <span data-ttu-id="13a70-470">Kommandot använder **meddelande** parametern, men skriver inte ett meddelande till PowerShell-konsolen.</span><span class="sxs-lookup"><span data-stu-id="13a70-470">The command uses the **Message** parameter, but doesn't write a message to the PowerShell console.</span></span>

```powershell
Write-Verbose -Message "Verbose message test."
```

<span data-ttu-id="13a70-471">När den **utförliga** parametern används, skrivs meddelandet.</span><span class="sxs-lookup"><span data-stu-id="13a70-471">When the **Verbose** parameter is used, the message is written.</span></span>
...
<span data-ttu-id="13a70-472">Det här exemplet visar resultatet av värdet **Continue** .</span><span class="sxs-lookup"><span data-stu-id="13a70-472">This example shows the effect of the **Continue** value.</span></span> <span data-ttu-id="13a70-473">`$VerbosePreference`Variabeln är inställd på att **fortsätta** och meddelandet visas.</span><span class="sxs-lookup"><span data-stu-id="13a70-473">The `$VerbosePreference` variable is set to **Continue** and the message is displayed.</span></span>
...
<span data-ttu-id="13a70-474">I det här exemplet används **verbose** -parametern med värdet `$false` som åsidosätter **Continue** -värdet.</span><span class="sxs-lookup"><span data-stu-id="13a70-474">This example uses the **Verbose** parameter with a value of `$false` that overrides the **Continue** value.</span></span> <span data-ttu-id="13a70-475">Meddelandet visas inte.</span><span class="sxs-lookup"><span data-stu-id="13a70-475">The message isn't displayed.</span></span>

```powershell
Write-Verbose -Message "Verbose message test." -Verbose:$false
```

<span data-ttu-id="13a70-476">Det här exemplet visar resultatet av **stopp** värdet.</span><span class="sxs-lookup"><span data-stu-id="13a70-476">This example shows the effect of the **Stop** value.</span></span> <span data-ttu-id="13a70-477">`$VerbosePreference`Variabeln har angetts till **stoppa** och meddelandet visas.</span><span class="sxs-lookup"><span data-stu-id="13a70-477">The `$VerbosePreference` variable is set to **Stop** and the message is displayed.</span></span> <span data-ttu-id="13a70-478">Kommandot har stoppats.</span><span class="sxs-lookup"><span data-stu-id="13a70-478">The command is stopped.</span></span>
...
<span data-ttu-id="13a70-479">I det här exemplet används **verbose** -parametern med värdet `$false` som åsidosätter **stopp** värdet.</span><span class="sxs-lookup"><span data-stu-id="13a70-479">This example uses the **Verbose** parameter with a value of `$false` that overrides the **Stop** value.</span></span> <span data-ttu-id="13a70-480">Meddelandet visas inte.</span><span class="sxs-lookup"><span data-stu-id="13a70-480">The message isn't displayed.</span></span>

```powershell
Write-Verbose -Message "Verbose message test." -Verbose:$false
```

<span data-ttu-id="13a70-481">I det här exemplet visas resultatet av **förfrågning** svärdet.</span><span class="sxs-lookup"><span data-stu-id="13a70-481">This example shows the effect of the **Inquire** value.</span></span> <span data-ttu-id="13a70-482">`$VerbosePreference`Variabeln är inställd på **fråga**.</span><span class="sxs-lookup"><span data-stu-id="13a70-482">The `$VerbosePreference` variable is set to **Inquire**.</span></span> <span data-ttu-id="13a70-483">Meddelandet visas och användaren uppmanas att bekräfta.</span><span class="sxs-lookup"><span data-stu-id="13a70-483">The message is displayed and the user is prompted for confirmation.</span></span>
...
<span data-ttu-id="13a70-484">I det här exemplet används **verbose** -parametern med värdet `$false` som åsidosätter värdet för **förfrågan** .</span><span class="sxs-lookup"><span data-stu-id="13a70-484">This example uses the **Verbose** parameter with a value of `$false` that overrides the **Inquire** value.</span></span> <span data-ttu-id="13a70-485">Användaren tillfrågas inte och meddelandet visas inte.</span><span class="sxs-lookup"><span data-stu-id="13a70-485">The user isn't prompted and the message isn't displayed.</span></span>

```powershell
Write-Verbose -Message "Verbose message test." -Verbose:$false
```

### <a name="warningpreference"></a><span data-ttu-id="13a70-486">\$WarningPreference</span><span class="sxs-lookup"><span data-stu-id="13a70-486">\$WarningPreference</span></span>

<span data-ttu-id="13a70-487">Fastställer hur PowerShell svarar på varnings meddelanden som genereras av ett skript, en cmdlet eller en provider, till exempel de meddelanden som genereras av cmdleten [Write-Warning](xref:Microsoft.PowerShell.Utility.Write-Warning) .</span><span class="sxs-lookup"><span data-stu-id="13a70-487">Determines how PowerShell responds to warning messages generated by a script, cmdlet, or provider, such as the messages generated by the [Write-Warning](xref:Microsoft.PowerShell.Utility.Write-Warning) cmdlet.</span></span>

<span data-ttu-id="13a70-488">Som standard visas varnings meddelanden och körningen fortsätter, men du kan ändra det här beteendet genom att ändra värdet för `$WarningPreference` .</span><span class="sxs-lookup"><span data-stu-id="13a70-488">By default, warning messages are displayed and execution continues, but you can change this behavior by changing the value of `$WarningPreference`.</span></span>

<span data-ttu-id="13a70-489">Du kan använda parametern **WarningAction** common för en cmdlet för att avgöra hur PowerShell svarar på varningar från ett visst kommando.</span><span class="sxs-lookup"><span data-stu-id="13a70-489">You can use the **WarningAction** common parameter of a cmdlet to determine how PowerShell responds to warnings from a particular command.</span></span> <span data-ttu-id="13a70-490">Mer information finns i [about_CommonParameters](about_CommonParameters.md).</span><span class="sxs-lookup"><span data-stu-id="13a70-490">For more information, see [about_CommonParameters](about_CommonParameters.md).</span></span>

<span data-ttu-id="13a70-491">Giltiga värden är följande:</span><span class="sxs-lookup"><span data-stu-id="13a70-491">The valid values are as follows:</span></span>

- <span data-ttu-id="13a70-492">**Stop** : visar varnings meddelandet och ett fel meddelande och slutar sedan att köra.</span><span class="sxs-lookup"><span data-stu-id="13a70-492">**Stop** : Displays the warning message and an error message and then stops executing.</span></span>
- <span data-ttu-id="13a70-493">**Fråga** : visar varnings meddelandet och frågar sedan om behörighet att fortsätta.</span><span class="sxs-lookup"><span data-stu-id="13a70-493">**Inquire** : Displays the warning message and then prompts for permission to continue.</span></span>
- <span data-ttu-id="13a70-494">**Fortsätt** : (standard) visar varnings meddelandet och fortsätter sedan att köra.</span><span class="sxs-lookup"><span data-stu-id="13a70-494">**Continue** : (Default) Displays the warning message and then continues executing.</span></span>
- <span data-ttu-id="13a70-495">**SilentlyContinue** : visar inte varnings meddelandet.</span><span class="sxs-lookup"><span data-stu-id="13a70-495">**SilentlyContinue** : Doesn't display the warning message.</span></span> <span data-ttu-id="13a70-496">Fortsätter att köra.</span><span class="sxs-lookup"><span data-stu-id="13a70-496">Continues executing.</span></span>

#### <a name="examples"></a><span data-ttu-id="13a70-497">Exempel</span><span class="sxs-lookup"><span data-stu-id="13a70-497">Examples</span></span>

<span data-ttu-id="13a70-498">I de här exemplen visas effekterna av de olika värdena i `$WarningPreference` .</span><span class="sxs-lookup"><span data-stu-id="13a70-498">These examples show the effect of the different values of `$WarningPreference`.</span></span>
<span data-ttu-id="13a70-499">Parametern **WarningAction** åsidosätter Preference-värdet.</span><span class="sxs-lookup"><span data-stu-id="13a70-499">The **WarningAction** parameter overrides the preference value.</span></span>

<span data-ttu-id="13a70-500">I det här exemplet visas resultatet av standardvärdet, **Fortsätt**.</span><span class="sxs-lookup"><span data-stu-id="13a70-500">This example shows the effect of the default value, **Continue**.</span></span>
...
<span data-ttu-id="13a70-501">I det här exemplet används parametern **WarningAction** med värdet **SilentlyContinue** för att förhindra varningen.</span><span class="sxs-lookup"><span data-stu-id="13a70-501">This example uses the **WarningAction** parameter with the value **SilentlyContinue** to suppress the warning.</span></span> <span data-ttu-id="13a70-502">Meddelandet visas inte.</span><span class="sxs-lookup"><span data-stu-id="13a70-502">The message isn't displayed.</span></span>
...
<span data-ttu-id="13a70-503">I det här exemplet ändras `$WarningPreference` variabeln till **SilentlyContinue** -värdet.</span><span class="sxs-lookup"><span data-stu-id="13a70-503">This example changes the `$WarningPreference` variable to the **SilentlyContinue** value.</span></span> <span data-ttu-id="13a70-504">Meddelandet visas inte.</span><span class="sxs-lookup"><span data-stu-id="13a70-504">The message isn't displayed.</span></span>
...
<span data-ttu-id="13a70-505">I det här exemplet används parametern **WarningAction** för att stoppa när en varning genereras.</span><span class="sxs-lookup"><span data-stu-id="13a70-505">This example uses the **WarningAction** parameter to stop when a warning is generated.</span></span>
...
<span data-ttu-id="13a70-506">Det här exemplet ändrar `$WarningPreference` variabeln till **fråge** värdet.</span><span class="sxs-lookup"><span data-stu-id="13a70-506">This example changes the `$WarningPreference` variable to the **Inquire** value.</span></span> <span data-ttu-id="13a70-507">Användaren uppmanas att bekräfta.</span><span class="sxs-lookup"><span data-stu-id="13a70-507">The user is prompted for confirmation.</span></span>
...
<span data-ttu-id="13a70-508">I det här exemplet används parametern **WarningAction** med värdet **SilentlyContinue**.</span><span class="sxs-lookup"><span data-stu-id="13a70-508">This example uses the **WarningAction** parameter with the value **SilentlyContinue**.</span></span> <span data-ttu-id="13a70-509">Kommandot fortsätter att köras och inget meddelande visas.</span><span class="sxs-lookup"><span data-stu-id="13a70-509">The command continues to execute and no message is displayed.</span></span>
...
<span data-ttu-id="13a70-510">Det här exemplet ändrar `$WarningPreference` värdet för att **stoppa**.</span><span class="sxs-lookup"><span data-stu-id="13a70-510">This example changes the `$WarningPreference` value to **Stop**.</span></span>
...
<span data-ttu-id="13a70-511">I det här exemplet används **WarningAction** med **frågans** värde.</span><span class="sxs-lookup"><span data-stu-id="13a70-511">This example uses the **WarningAction** with the **Inquire** value.</span></span> <span data-ttu-id="13a70-512">Användaren tillfrågas när en varning inträffar.</span><span class="sxs-lookup"><span data-stu-id="13a70-512">The user is prompted when a warning occurs.</span></span>
...
### <a name="whatifpreference"></a><span data-ttu-id="13a70-513">\$WhatIfPreference</span><span class="sxs-lookup"><span data-stu-id="13a70-513">\$WhatIfPreference</span></span>

<span data-ttu-id="13a70-514">Bestämmer om **whatIf** automatiskt aktive ras för varje kommando som stöder det.</span><span class="sxs-lookup"><span data-stu-id="13a70-514">Determines whether **WhatIf** is automatically enabled for every command that supports it.</span></span> <span data-ttu-id="13a70-515">När **whatIf** är aktiverat rapporterar cmdleten den förväntade resultatet av kommandot, men kör inte kommandot.</span><span class="sxs-lookup"><span data-stu-id="13a70-515">When **WhatIf** is enabled, the cmdlet reports the expected effect of the command, but doesn't execute the command.</span></span>

<span data-ttu-id="13a70-516">Giltiga värden är följande:</span><span class="sxs-lookup"><span data-stu-id="13a70-516">The valid values are as follows:</span></span>

- <span data-ttu-id="13a70-517">**Falskt** ( **0** , ej aktiverat): (standard) **whatIf** aktive ras inte automatiskt.</span><span class="sxs-lookup"><span data-stu-id="13a70-517">**False** ( **0** , not enabled): (Default) **WhatIf** isn't automatically enabled.</span></span> <span data-ttu-id="13a70-518">Använd cmdletens **whatIf** -parameter om du vill aktivera det manuellt.</span><span class="sxs-lookup"><span data-stu-id="13a70-518">To enable it manually, use the cmdlet's **WhatIf** parameter.</span></span>
- <span data-ttu-id="13a70-519">**Sant** ( **1** , aktive rad): **whatIf** aktive ras automatiskt för alla kommandon som stöder det.</span><span class="sxs-lookup"><span data-stu-id="13a70-519">**True** ( **1** , enabled): **WhatIf** is automatically enabled on any command that supports it.</span></span> <span data-ttu-id="13a70-520">Användare kan använda parametern **whatIf** med värdet **false** för att inaktivera den manuellt, till exempel `-WhatIf:$false` .</span><span class="sxs-lookup"><span data-stu-id="13a70-520">Users can use the **WhatIf** parameter with a value of **False** to disable it manually, such as `-WhatIf:$false`.</span></span>

#### <a name="examples"></a><span data-ttu-id="13a70-521">Exempel</span><span class="sxs-lookup"><span data-stu-id="13a70-521">Examples</span></span>

<span data-ttu-id="13a70-522">I de här exemplen visas effekterna av de olika värdena i `$WhatIfPreference` .</span><span class="sxs-lookup"><span data-stu-id="13a70-522">These examples show the effect of the different values of `$WhatIfPreference`.</span></span>
<span data-ttu-id="13a70-523">De visar hur du använder parametern **whatIf** för att åsidosätta Preference-värdet för ett speciellt kommando.</span><span class="sxs-lookup"><span data-stu-id="13a70-523">They show how to use the **WhatIf** parameter to override the preference value for a specific command.</span></span>

<span data-ttu-id="13a70-524">Det här exemplet visar resultatet av `$WhatIfPreference` variabeln som angetts till standardvärdet, **false**.</span><span class="sxs-lookup"><span data-stu-id="13a70-524">This example shows the effect of the `$WhatIfPreference` variable set to the default value, **False**.</span></span> <span data-ttu-id="13a70-525">Används `Get-ChildItem` för att kontrol lera att filen finns.</span><span class="sxs-lookup"><span data-stu-id="13a70-525">Use `Get-ChildItem` to verify that the file exists.</span></span>
<span data-ttu-id="13a70-526">`Remove-Item` tar bort filen.</span><span class="sxs-lookup"><span data-stu-id="13a70-526">`Remove-Item` deletes the file.</span></span> <span data-ttu-id="13a70-527">När filen har tagits bort kan du kontrol lera borttagningen med `Get-ChildItem` .</span><span class="sxs-lookup"><span data-stu-id="13a70-527">After the file is deleted, you can verify the deletion with `Get-ChildItem`.</span></span>
...
<span data-ttu-id="13a70-528">Det här exemplet visar effekterna av att använda parametern **whatIf** när värdet för `$WhatIfPreference` är **false**.</span><span class="sxs-lookup"><span data-stu-id="13a70-528">This example shows the effect of using the **WhatIf** parameter when the value of `$WhatIfPreference` is **False**.</span></span>

<span data-ttu-id="13a70-529">Kontrol lera att filen finns.</span><span class="sxs-lookup"><span data-stu-id="13a70-529">Verify that the file exists.</span></span>
...
<span data-ttu-id="13a70-530">Använd parametern **whatIf** för att avgöra resultatet av försöket att ta bort filen.</span><span class="sxs-lookup"><span data-stu-id="13a70-530">Use the **WhatIf** parameter to determine the result of attempting to delete the file.</span></span>
...
<span data-ttu-id="13a70-531">Kontrol lera att filen inte har tagits bort.</span><span class="sxs-lookup"><span data-stu-id="13a70-531">Verify that the file wasn't deleted.</span></span>
...
<span data-ttu-id="13a70-532">I det här exemplet visas resultatet av `$WhatIfPreference` variabeln som anges till värdet, **Sant**.</span><span class="sxs-lookup"><span data-stu-id="13a70-532">This example shows the effect of the `$WhatIfPreference` variable set to the value, **True**.</span></span> <span data-ttu-id="13a70-533">När du använder `Remove-Item` för att ta bort en fil visas filens sökväg, men filen tas inte bort.</span><span class="sxs-lookup"><span data-stu-id="13a70-533">When you use `Remove-Item` to delete a file, the file's path is displayed, but the file isn't deleted.</span></span>

<span data-ttu-id="13a70-534">Försök att ta bort en fil.</span><span class="sxs-lookup"><span data-stu-id="13a70-534">Attempt to delete a file.</span></span> <span data-ttu-id="13a70-535">Ett meddelande visas om vad som skulle hända om `Remove-Item` kördes, men filen inte tas bort.</span><span class="sxs-lookup"><span data-stu-id="13a70-535">A message is displayed about what would happen if `Remove-Item` was run, but the file isn't deleted.</span></span>
...
<span data-ttu-id="13a70-536">Används `Get-ChildItem` för att kontrol lera att filen inte har tagits bort.</span><span class="sxs-lookup"><span data-stu-id="13a70-536">Use `Get-ChildItem` to verify that the file wasn't deleted.</span></span>
...
<span data-ttu-id="13a70-537">I det här exemplet visas hur du tar bort en fil när värdet `$WhatIfPreference` är **True**.</span><span class="sxs-lookup"><span data-stu-id="13a70-537">This example shows how to delete a file when the value of `$WhatIfPreference` is **True**.</span></span> <span data-ttu-id="13a70-538">Den använder parametern **whatIf** med värdet `$false` .</span><span class="sxs-lookup"><span data-stu-id="13a70-538">It uses the **WhatIf** parameter with a value of `$false`.</span></span> <span data-ttu-id="13a70-539">Används `Get-ChildItem` för att kontrol lera att filen har tagits bort.</span><span class="sxs-lookup"><span data-stu-id="13a70-539">Use `Get-ChildItem` to verify the file was deleted.</span></span>
...
<span data-ttu-id="13a70-540">Följande är exempel på den `Get-Process` cmdlet som inte stöder **whatIf** och `Stop-Process` som stöder **whatIf**.</span><span class="sxs-lookup"><span data-stu-id="13a70-540">The following are examples of the `Get-Process` cmdlet that doesn't support **WhatIf** and `Stop-Process` that does support **WhatIf**.</span></span> <span data-ttu-id="13a70-541">`$WhatIfPreference`Variabelns värde är **Sant**.</span><span class="sxs-lookup"><span data-stu-id="13a70-541">The `$WhatIfPreference` variable's value is **True**.</span></span>

<span data-ttu-id="13a70-542">`Get-Process` stöder inte **whatIf**.</span><span class="sxs-lookup"><span data-stu-id="13a70-542">`Get-Process` doesn't support **WhatIf**.</span></span> <span data-ttu-id="13a70-543">När kommandot körs visas **Winword** -processen.</span><span class="sxs-lookup"><span data-stu-id="13a70-543">When the command is executed, it displays the **Winword** process.</span></span>
...
<span data-ttu-id="13a70-544">`Stop-Process` stöder **whatIf**.</span><span class="sxs-lookup"><span data-stu-id="13a70-544">`Stop-Process` does support **WhatIf**.</span></span> <span data-ttu-id="13a70-545">**Winword** -processen har inte stoppats.</span><span class="sxs-lookup"><span data-stu-id="13a70-545">The **Winword** process isn't stopped.</span></span>
...
<span data-ttu-id="13a70-546">Du kan åsidosätta `Stop-Process` **whatIf** -beteendet med hjälp av parametern **whatIf** med värdet `$false` .</span><span class="sxs-lookup"><span data-stu-id="13a70-546">You can override the `Stop-Process` **WhatIf** behavior by using the **WhatIf** parameter with a value of `$false`.</span></span> <span data-ttu-id="13a70-547">**Winword** -processen har stoppats.</span><span class="sxs-lookup"><span data-stu-id="13a70-547">The **Winword** process is stopped.</span></span>

```powershell
Stop-Process -Name Winword -WhatIf:$false
```

<span data-ttu-id="13a70-548">Använd om du vill kontrol lera att **Winword** -processen har stoppats `Get-Process` .</span><span class="sxs-lookup"><span data-stu-id="13a70-548">To verify that the **Winword** process was stopped, use `Get-Process`.</span></span>
...
## <a name="see-also"></a><span data-ttu-id="13a70-549">Se även</span><span class="sxs-lookup"><span data-stu-id="13a70-549">See also</span></span>

[<span data-ttu-id="13a70-550">about_Automatic_Variables</span><span class="sxs-lookup"><span data-stu-id="13a70-550">about_Automatic_Variables</span></span>](about_Automatic_Variables.md)

[<span data-ttu-id="13a70-551">about_CommonParameters</span><span class="sxs-lookup"><span data-stu-id="13a70-551">about_CommonParameters</span></span>](about_CommonParameters.md)

[<span data-ttu-id="13a70-552">about_Environment_Variables</span><span class="sxs-lookup"><span data-stu-id="13a70-552">about_Environment_Variables</span></span>](about_Environment_Variables.md)

[<span data-ttu-id="13a70-553">about_Profiles</span><span class="sxs-lookup"><span data-stu-id="13a70-553">about_Profiles</span></span>](about_Profiles.md)

[<span data-ttu-id="13a70-554">about_Remote</span><span class="sxs-lookup"><span data-stu-id="13a70-554">about_Remote</span></span>](about_Remote.md)

[<span data-ttu-id="13a70-555">about_Scopes</span><span class="sxs-lookup"><span data-stu-id="13a70-555">about_Scopes</span></span>](about_Scopes.md)

[<span data-ttu-id="13a70-556">about_Variables</span><span class="sxs-lookup"><span data-stu-id="13a70-556">about_Variables</span></span>](about_Variables.md)
