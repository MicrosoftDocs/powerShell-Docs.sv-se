...
Du kan använda parametrarna för `Get-Random` för att ange ett start nummer, lägsta och högsta värde, antalet objekt som returneras från en inskickad samling och hela samlingen i en slumpmässig ordning.
...
Från och med PowerShell 7,1 kan du använda parametern **blanda** för att returnera hela samlingen i slumpmässig ordning.
...
### Exempel 12: Använd Count-parametern

Du kan nu använda **Count** -parametern utan rör objekt till `Get-Random` .
I följande exempel hämtas tre slumptal som är mindre än 10.
...
`Get-Random` returnerar antingen en `a` tom sträng eller `$null` . Den tomma STING visas som en tom rad och `$null` återgår till en PowerShell-prompt.
...
Från och med PowerShell 7 accepterar parametern **InputObject** matriser som kan innehålla en tom sträng eller `$null` . Matrisen kan skickas nedåt i pipeline eller som ett **InputObject** -parameter värde.
