...
---
# <a name="resource-dependencies-using-dependson"></a><span data-ttu-id="36100-103">Resursberoenden med DependsOn</span><span class="sxs-lookup"><span data-stu-id="36100-103">Resource dependencies using DependsOn</span></span>

<span data-ttu-id="36100-104">När du skriver [konfigurationer](configurations.md)lägger du till [resurs block](../resources/resources.md) för att konfigurera aspekter av en målnod.</span><span class="sxs-lookup"><span data-stu-id="36100-104">When you write [Configurations](configurations.md), you add [Resource blocks](../resources/resources.md) to configure aspects of a target Node.</span></span> <span data-ttu-id="36100-105">När du fortsätter att lägga till resurs block kan dina konfigurationer växa ganska stora och besvärliga att hantera.</span><span class="sxs-lookup"><span data-stu-id="36100-105">As you continue to add Resource blocks, your Configurations can grow quite large and cumbersome to manage.</span></span> <span data-ttu-id="36100-106">En sådan utmaning är den tillämpade ordningen på dina resurs block.</span><span class="sxs-lookup"><span data-stu-id="36100-106">One such challenge is the applied order of your resource blocks.</span></span> <span data-ttu-id="36100-107">Resurser tillämpas vanligt vis i den ordning som de definieras i konfigurationen.</span><span class="sxs-lookup"><span data-stu-id="36100-107">Typically resources are applied in the order they are defined within the Configuration.</span></span> <span data-ttu-id="36100-108">När konfigurationen blir större och mer komplex kan du använda `DependsOn` nyckeln för att ändra den tillämpade ordningen för dina resurser genom att ange att en resurs är beroende av en annan resurs.</span><span class="sxs-lookup"><span data-stu-id="36100-108">As your Configuration grows larger and more complex, you can use the `DependsOn` key to change the applied order of your resources by specifying that a resource depends on another resource.</span></span>

<span data-ttu-id="36100-109">`DependsOn` Nyckeln kan användas i alla resurs block.</span><span class="sxs-lookup"><span data-stu-id="36100-109">The `DependsOn` key can be used in any Resource block.</span></span> <span data-ttu-id="36100-110">Den definieras med samma nyckel/värde-mekanism som andra resurs nycklar.</span><span class="sxs-lookup"><span data-stu-id="36100-110">It is defined with the same key/value mechanism as other Resource keys.</span></span> <span data-ttu-id="36100-111">`DependsOn` Nyckeln förväntar sig en sträng mat ris med följande syntax.</span><span class="sxs-lookup"><span data-stu-id="36100-111">The `DependsOn` key expects an array of strings with the following syntax.</span></span>

```
DependsOn = '[<Resource Type>]<Resource Name>', '[<Resource Type>]<Resource Name'
```

<span data-ttu-id="36100-112">I följande exempel konfigureras en brand Väggs regel när du har aktiverat och konfigurerat den offentliga profilen.</span><span class="sxs-lookup"><span data-stu-id="36100-112">The following example configures a firewall rule after enabling and configuring the public profile.</span></span>
...
<span data-ttu-id="36100-113">När du tillämpar konfigurationen kommer brand Väggs profilen alltid att konfigureras först oavsett vilken ordning resurs block definieras.</span><span class="sxs-lookup"><span data-stu-id="36100-113">When you apply the Configuration, the firewall profile will always be configured first regardless of which order the Resource blocks are defined.</span></span> <span data-ttu-id="36100-114">Om du tillämpar konfigurationen måste du anteckna vilka mål noder som är befintliga, så att du kan återställa om du vill.</span><span class="sxs-lookup"><span data-stu-id="36100-114">If you apply the Configuration, be sure to note your target Nodes existing Configuration so you can revert if desired.</span></span>
...
<span data-ttu-id="36100-115">Detta säkerställer också att om **FirewallProfile** -resursen Miss lyckas av någon anledning, körs inte **brand Väggs** blocket trots att det definierades först.</span><span class="sxs-lookup"><span data-stu-id="36100-115">This also ensures that if the **FirewallProfile** resource fails for any reason, the **Firewall** block will not execute even though it was defined first.</span></span> <span data-ttu-id="36100-116">`DependsOn` Nyckeln ger större flexibilitet vid gruppering av resurs block och säkerställer att beroenden löses innan en resurs körs.</span><span class="sxs-lookup"><span data-stu-id="36100-116">The `DependsOn` key allows more flexibility in grouping resource blocks and ensuring that dependencies are resolved before a Resource executes.</span></span>

<span data-ttu-id="36100-117">I mer avancerade konfigurationer kan du också använda [internode-beroende](crossNodeDependencies.md) för att tillåta ännu mer detaljerad kontroll (till exempel se till att en domänkontrollant har kon figurer ATS innan du ansluter till en klient i domänen).</span><span class="sxs-lookup"><span data-stu-id="36100-117">In more advanced Configurations, you can also use [Cross Node Dependency](crossNodeDependencies.md) to allow even more granular control (For example, ensuring a domain controller is configured before joining a client to the domain).</span></span>

## <a name="cleaning-up"></a><span data-ttu-id="36100-118">Rensar</span><span class="sxs-lookup"><span data-stu-id="36100-118">Cleaning Up</span></span>

<span data-ttu-id="36100-119">Om du använde konfigurationen ovan kan du byta nycklar för att ångra eventuella ändringar.</span><span class="sxs-lookup"><span data-stu-id="36100-119">If you applied the Configuration above, you can reverse keys to undo any changes.</span></span> <span data-ttu-id="36100-120">I exemplet ovan inaktive ras brand Väggs regeln och-profilen om du anger den **aktiverade** nyckeln till false.</span><span class="sxs-lookup"><span data-stu-id="36100-120">In the above example, setting the **Enabled** key to false will disable the firewall rule and profile.</span></span> <span data-ttu-id="36100-121">Du bör ändra exemplet efter behov för att matcha målets tidigare konfigurerade tillstånd.</span><span class="sxs-lookup"><span data-stu-id="36100-121">You should modify the example as needed to match your target Node's previous configured state.</span></span>
...
## <a name="see-also"></a><span data-ttu-id="36100-122">Se även</span><span class="sxs-lookup"><span data-stu-id="36100-122">See also</span></span>

- [<span data-ttu-id="36100-123">Använda kors nods beroenden</span><span class="sxs-lookup"><span data-stu-id="36100-123">Use Cross Node Dependencies</span></span>](./crossNodeDependencies.md)
