---
ms.date: 2017-06-05
keywords: PowerShell-cmdlet
title: "Referens för installation"
ms.openlocfilehash: 39da5f2b524d908270066926ff2cdc56a665fe35
ms.sourcegitcommit: 6aed37d7f0c9652ae09bb8c11928da7e4783ed7f
ms.translationtype: MT
ms.contentlocale: sv-SE
ms.lasthandoff: 01/10/2018
---
# <a name="setup-guide"></a>Guiden

- [Installera PowerShell Core på macOS- och Linux](Installing-PowerShell-Core-on-macOS-and-Linux.md)
- [Installera PowerShell Core i Windows](Installing-PowerShell-Core-on-Windows.md)
- [Installera Windows PowerShell](Installing-Windows-PowerShell.md)
- [Installera Windows PowerShell 2.0-motorn](Installing-the-Windows-PowerShell-2.0-Engine.md)
- [Installera Windows PowerShell SDK:n](Installing-the-Windows-PowerShell-SDK.md)
- [Referens för installation](setup-reference.md)
- [Hjälpmedelsfunktioner i Windows PowerShell ISE](Accessibility-in-Windows-PowerShell-ISE.md)
- [Starta 32-bitars Version av Windows PowerShell](Starting-the-32-Bit-Version-of-Windows-PowerShell.md)
- [Starta Windows PowerShell 2.0-motorn](Starting-the-Windows-PowerShell-2.0-Engine.md)
- [Starta Windows PowerShell på tidigare versioner av Windows](Starting-Windows-PowerShell-on-Earlier-Versions-of-Windows.md)
- [Starta Windows PowerShell](Starting-Windows-PowerShell.md)
- [Systemkrav för Windows PowerShell](Windows-PowerShell-System-Requirements.md)
- [WinRMSecurity](WinRMSecurity.md)
