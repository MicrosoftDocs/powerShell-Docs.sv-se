...
När du tittar på säkerhets gransknings händelser och program händelse loggar ser du att varje JEA-användarsession har ett unikt virtuellt konto. Det här unika kontot hjälper dig att spåra användar åtgärder i en JEA-slutpunkt tillbaka till den ursprungliga användare som körde kommandot. Namn på virtuella konton följer formatet `WinRM Virtual Users\WinRM_VA_<ACCOUNTNUMBER>_<DOMAIN>_<sAMAccountName>` till exempel, om User **Alice** i domän **contoso** startar om en tjänst i en Jea-slutpunkt, kommer det användar namn som är associerat med alla Service Control manager-händelser att `WinRM Virtual Users\WinRM_VA_1_contoso_alice`.
...
Med **standard kör som-konton** kan du ange vilket användar konto som hela PowerShell-sessionen ska köras under. Sessionshantering som använder fasta **Kör som-** konton (med parametern `-RunAsCredential`) är inte Jea-medvetna. Roll definitionerna fungerar inte längre som förväntat. Alla användare som har behörighet att komma åt slut punkten tilldelas samma roll.
