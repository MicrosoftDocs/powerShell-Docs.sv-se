...
Här är den fullständiga exempel koden för VB.NET för StopProc04-exempel-cmdlet: en. Det här är koden för cmdleten `Stop-Process` som beskrivs i [lägga till parameter uppsättningar i en cmdlet](../cmdlet/adding-parameter-sets-to-a-cmdlet.md). Cmdleten `Stop-Process` är utformad för att stoppa processer som hämtas med cmdleten Get-proc (beskrivs i [skapa din första cmdlet](../cmdlet/creating-a-cmdlet-without-parameters.md)).
