...
> [!NOTE]
> Du kan hämta C#-källfilen (AccessDBSampleProvider05.cs) för den här providern med hjälp av Microsoft Windows Software Development Kit för Windows Vista och .NET Framework 3,0 Runtime-komponenter. Instruktioner för hämtning finns i [Installera Windows PowerShell och ladda ned Windows POWERSHELL SDK](/powershell/scripting/developer/installing-the-windows-powershell-sdk).
...
Åsidosättningar av den här metoden bör som standard inte flytta objekt över befintliga objekt om inte egenskapen [system. Management. Automation. Provider. Cmdletprovider. Force *](/dotnet/api/System.Management.Automation.Provider.CmdletProvider.Force) anges till `true` . Till exempel kopierar inte fil systemets Provider c:\temp\abc.txt över en befintlig c:\bar.txt-fil om inte egenskapen [system. Management. Automation. Provider. Cmdletprovider. Force *](/dotnet/api/System.Management.Automation.Provider.CmdletProvider.Force) anges till `true` . Om sökvägen som anges i `destination` parametern finns och är en behållare, krävs inte egenskapen [system. Management. Automation. Provider. Cmdletprovider. Force *](/dotnet/api/System.Management.Automation.Provider.CmdletProvider.Force) . I det här fallet ska [system. Management. Automation. Provider. Navigationcmdletprovider. Moveitem *](/dotnet/api/System.Management.Automation.Provider.NavigationCmdletProvider.MoveItem) flytta det objekt som anges av `path` parametern till den behållare som anges av `destination` parametern som underordnad.
...
2. Kör nu `Get-Childitem` cmdleten för att hämta en lista över databas objekt, som är tillgängliga databas tabeller. För varje tabell hämtar denna cmdlet också antalet tabell rader.
...
3. Använd `Set-Location` cmdleten igen för att ange platsen för data tabellen anställda.
...
5. Använd nu `Get-Childitem` cmdlet-skickas till `Format-Table` cmdleten. Den här uppsättningen cmdlet: ar hämtar objekten för data tabellen anställda, som är tabell rader. De formateras som de anges av `Format-Table` cmdleten.
...
7. Använd `Get-Item` cmdleten igen för att hämta medarbetar data för objekten på rad 0.
