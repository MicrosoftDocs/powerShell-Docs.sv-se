...
- Ignorera växeln `-ExecutionPolicy` när du kör PowerShell på plattformar som inte är Windows-plattformar eftersom skript signering inte stöds för närvarande. (#3481)
- Fast ConsoleHost för att `NoEcho` respektera UNIX-plattformar. (#3801)
...
Miljövariabler är till exempel Skift läges känsliga på macOS och Linux, vilket innebär `PSModulePath` att miljövariabeln inte har standardiserats. (#3255) `Import-Module` är Skift läges okänslig när den använder en fil Sök väg för att fastställa modulens namn. (#5097)
...
- Den första positions parametern har ändrats `-Command` från `-File`till. Den här ändringen åtgärdar användningen `#!` av (aka som en Shebang) i PowerShell-skript som körs från icke-PowerShell-gränssnitt på andra plattformar än Windows-plattformar. Det innebär också att du kan köra kommandon som `pwsh foo.ps1` eller `pwsh fooScript` utan att `-File`ange. Den här ändringen kräver dock att du uttryckligen anger `-c` eller `-Command` när du försöker köra kommandon som `pwsh.exe -Command Get-Command`.
  (#4019)
- PowerShell `-i` -kärnan godkänner (eller `-Interactive`) växeln för att ange ett interaktivt gränssnitt.
  (#3558) Detta gör att PowerShell kan användas som standard gränssnitt på UNIX-plattformar.
- Parametrar `-importsystemmodules` och `-psconsoleFile` från `pwsh.exe`har tagits bort. (#4995)
...
- Parametern `-WindowStyle` har lagts till i Windows. (#4573) På samma sätt är paketbaserade installationer som inte är Windows-plattformar uppdateringar på plats.
...
Det innebär att du kan använda cmdletar som `Enter-PSSession` och `New-PSSession` och autentisera med SSH. Allt du behöver göra är att registrera PowerShell som ett under system med en OpenSSH-baserad SSH-server och du kan använda dina befintliga SSH-baserade autentiserande mekanismer (t. ex. lösen ord eller privata `PSSession` nycklar) med traditionella semantik.
...
Tidigare var Windows PowerShell-cmdletar som, `Get-Content` `Set-Content` som använder olika kodningar, t. ex. ASCII och UTF-16. Var Ian sen i kodnings standardvärden skapade problem när du blandar cmdletar utan att ange en kodning.
...
Dessa cmdletar har också uppdaterats så att `-Encoding` parametern kan accepteras `System.Text.Encoding`universellt.
...
`New-ModuleManifest`cmdleten har ingen **encoding** -parameter. Kodningen för modul manifest filen (. psd1) som skapats med `New-ModuleManifest` cmdleten är beroende av miljön: om den är PowerShell Core som körs på Linux är kodningen UTF-8 (ingen struktur). annars är UTF-16 (med struktur). (#3940)

## <a name="support-backgrounding-of-pipelines-with-ampersand--3360"></a>Stöd för pipelines med et-tecken (`&`) (#3360)

Om `&` du lägger till slutet av en pipeline körs pipelinen som ett PowerShell-jobb. När en pipeline har ett bakgrunds objekt returneras ett jobb objekt. När pipelinen körs som ett jobb kan alla standard `*-Job` -cmdlets användas för att hantera jobbet. Variabler (ignorerar verksamhetsspecifika variabler) som används i pipelinen kopieras automatiskt till jobbet så `Copy-Item $foo $bar &` att bara fungerar. Jobbet körs också i den aktuella katalogen i stället för användarens Hem Katalog. Mer information om PowerShell-jobb finns [about_Jobs](/powershell/module/microsoft.powershell.core/about/about_jobs).
...
- `ModuleVersion` Standardvärdet `New-ModuleManifest` har `0.0.1` ändrats i så att det överensstämmer med SemVer. (#4842) (Tack [@LDSpits](https://github.com/LDSpits))
...
- Aktive rad jämförelse `SemanticVersion` mellan en instans `Version` och en instans som endast är `Major` konstruerad med och `Minor` versions värden.
...
- Stöd har lagts till för konvertering av uppräkningar till sträng ( [@KirkMunro](https://github.com/KirkMunro)#4318) (tack)
- Fast data mat ris med en generisk samling. (#3170)
- Tecken intervallets överbelastning till `..` operatorn har `'a'..'z'` lagts till, så returnerar tecken från "a" till "ö". (#5026) (Tack [@IISResetMe](https://github.com/IISResetMe)!)
...
- Aktivera användning av alternativet "Singleline, Multiline" i Split-operatorn (#4721) [@iSazonov](https://github.com/iSazonov)(tack)

## <a name="engine-updates"></a>Motor uppdateringar

- `$PSVersionTable`har fyra nya egenskaper:
  - `PSEdition`: Detta är inställt `Core` på på PowerShell `Desktop` Core och i Windows PowerShell
  ...
  - `Platform`: `[System.Environment]::OSVersion.Platform` Detta returneras av det är inställt `Win32NT` på på Windows `Unix` , MacOS och `Unix` Linux.
- `BuildVersion` Egenskapen har tagits bort `$PSVersionTable`från. Den här egenskapen är starkt knuten till Windows build-versionen. I stället rekommenderar vi att du använder `GitCommitId` för att hämta den exakta versionen av PowerShell Core. (#3877) (Tack till [@iSazonov](https://github.com/iSazonov)!)
- Ta `ClrVersion` bort egenskap `$PSVersionTable`från. Den här egenskapen är irrelevant för .NET Core och bevarades bara i .NET Core för speciella äldre syfte som inte är tillgängliga för PowerShell.
- Tre nya automatiska variabler har lagts till för att avgöra om PowerShell körs i ett angivet `$IsWindows`operativ `$IsMacOs`system: `$IsLinux`, och.
- Lägg `GitCommitId` till i PowerShell Core-banderoll. Nu behöver du inte köra `$PSVersionTable` så snart du startar PowerShell för att hämta versionen! (#3916) (Tack till [@iSazonov](https://github.com/iSazonov)!)
- Lägg till en JSON-konfigurationsfil `powershell.config.json` som `$PSHome` kallas i för att lagra vissa inställningar som krävs före start `ExecutionPolicy`tiden (t. ex.).
...
- Lägg `Get-Uptime` till `Microsoft.PowerShell.Utility`i.
- Lägg `Remove-Alias` till kommando. (#5143) (Tack [@PowershellNinja](https://github.com/PowershellNinja)!)
- Lägg `Remove-Service` till i Management-modulen. (#4858) (Tack [@joandrsn](https://github.com/joandrsn)!)
...
- Stöd för länk huvuds sid brytning i webb-cmdlets (#3828)
  - För `Invoke-WebRequest`, när svaret innehåller ett länk huvud, skapar vi en RelationLink-egenskap som en ord lista som representerar `rel` URL: er och attribut och ser till att URL: erna är absoluta för att det ska bli lättare för utvecklaren att använda.
  - För `Invoke-RestMethod`, när svaret innehåller ett länk huvud, visar vi en `-FollowRelLink` växel för att automatiskt `next` `rel` följa länkar tills de inte längre finns eller när vi har nått `-MaximumFollowRelLink` det valfria parameter värdet.
- Lägg `-CustomMethod` till parametern till webb-cmdletar för att tillåta verb som inte är standard. (#3142) (Tack till @Lee303!)
- Lägg `SslProtocol` till stöd för webb-cmdletar. (#5329) (Tack [@markekraus](https://github.com/markekraus)!)
- Lägg till multipart-stöd för Web-cmdletar. (#4782) (Tack [@markekraus](https://github.com/markekraus))
- Lägg `-NoProxy` till i webb-cmdlets så att de ignorerar den systemomfattande proxyinställningarna. (#3447) (Tack till [@TheFlyingCorpse](https://github.com/TheFlyingCorpse)!)
- Användar agenten för Web-cmdlets rapporterar nu operativ system plattformen (#4937) [@LDSpits](https://github.com/LDSpits)(tack)
- Lägg `-SkipHeaderValidation` till Switch till Web-cmdletar för att stödja tillägg av huvuden utan att verifiera Huvudvärdet. (#4085)
...
- Lägg till autentiseringsmetoder till webb-cmdletar. (#5052) (Tack [@markekraus](https://github.com/markekraus))
  - Lägg `-Authentication` till som innehåller tre alternativ: Basic, OAuth och Bearer.
  - Lägg `-Token` till för att hämta Bearer-token för OAuth-och Bearer-alternativ.
  - Lägg `-AllowUnencryptedAuthentication` till i kringgå autentisering som har angetts för något annat transport schema än https.
- Lägg `-ResponseHeadersVariable` till `Invoke-RestMethod` i för att aktivera insamlingen av svars rubriker.
...
- Ändra webb- `UserAgent` cmdletar `WindowsPowerShell` från `PowerShell`till. (#4914) (Tack [@markekraus](https://github.com/markekraus))
- Lägg till `ContentType` explicit identifiering `Invoke-RestMethod` till (#4692)
...
- Lägg `-AsHashtable` till `ConvertFrom-Json` för att returnera `Hashtable` en i stället. (#5043) (Tack [@bergmeister](https://github.com/bergmeister)!)
- Använd prettier-Formatter `ConvertTo-Json` med utdata. (#2787) (Tack till @kittholland!)
- Lägg `Jobject` till stöd för serialisering `ConvertTo-Json`i. (#5141)
- Korrigera `ConvertFrom-Json` för att deserialisera en sträng mat ris från pipelinen som tillsammans skapar en fullständig JSON-sträng. Detta åtgärdar vissa fall där newlines skulle bryta JSON-parsning.
  (#3823)
- Ta bort `AliasProperty "Count"` den definierade `System.Array`för. Detta tar bort den `Count` främmande egenskapen för `ConvertFrom-Json` vissa utdata. (#3231) (Tack till [@PetSerAl](https://github.com/PetSerAl)!)

### <a name="csv-cmdlets"></a>CSV-cmdletar

- `Import-Csv`stöder nu utökat logg fils format för W3C (#2482) ( [@iSazonov](https://github.com/iSazonov)tack!)
- Lägg `PSTypeName` till stöd `Import-Csv` för `ConvertFrom-Csv`och. (#5389) (Tack [@markekraus](https://github.com/markekraus)!)
...
- Lägg till `UserName`egenskaper `Description`, `DelayedAutoStart` `BinaryPathName`,, och `StartupType` till de `ServiceController` objekt som returnerades av `Get-Service`. (#4907) (Tack [@joandrsn](https://github.com/joandrsn))
- Lägg till funktioner för att ange `Set-Service` autentiseringsuppgifter för kommandot. (#4844) (Tack [@joandrsn](https://github.com/joandrsn))

### <a name="other-cmdlets"></a>Andra cmdletar

- Lägg till en parameter `Get-ChildItem` till `-FollowSymlink` anropad som passerar symlinks på begäran, med kontroller för länk slingor. (#4020)
...
- Lägg till stöd `Invoke-Item -Path <folder>`för. (#4262)
- Lägg `-Extension` till `-LeafBase` och växlar `Split-Path` till så att du kan dela upp sökvägar mellan fil namns tillägget och resten av fil namnet. (#2721) (Tack till [@powercode](https://github.com/powercode)!)
- Lägg till `-Top` parametrar `-Bottom` och `Sort-Object` för den översta/understa N-sorteringen
...
- Lägg `-NoNewLine` till växel `Out-String`för. (#5056) (Tack [@raghav710](https://github.com/raghav710))
- `Move-Item`cmdleten följer `-Include`parametrarna `-Exclude`, och `-Filter` . (#3878)
- Tillåt `*` användning i register Sök vägen för `Remove-Item`. (#4866)
- Lägg `-Title` till `Get-Credential` i och förena prompten på olika plattformar.
...
- Ta bort `-ShowWindow` växeln som inte stöds `Get-Help`från. (#4903)
- Korrigera `Get-Content -Delimiter` till inkludera inte avgränsaren i de mat ris element som returneras (#3706) ( [@mklement0](https://github.com/mklement0)tack)
- Lägg `Meta`till `Charset`, och `Transitional` parametrar för `ConvertTo-HTML` (#4184) (tack [@ergo3114](https://github.com/ergo3114))
- Lägg `WindowsUBR` till `WindowsVersion` och egenskaper `Get-ComputerInfo` som ska skapas
- Lägg `-Group` till parameter i`Get-Verb`
- Lägg `ShouldProcess` till stöd `New-FileCatalog` i `Test-FileCatalog` och ( `-WhatIf` korrigeringar `-Confirm`och). (#3074) (Tack till [@iSazonov](https://github.com/iSazonov)!)
- Lägg `-WhatIf` till växla `Start-Process` till cmdlet (#4735) ( [@sarithsutha](https://github.com/sarithsutha)tack)
- Lägg `ValidateNotNullOrEmpty` till för många befintliga parametrar.
...
- Fliken för att lägga till `-Property` en `Select-Object`hash-mängd för. (#3625) (Tack till [@powercode](https://github.com/powercode))
- Aktivera Autoavsluta av `-ExcludeProperty` `-ExpandProperty` `Select-Object`argument för och.
  (#3443) (Tack till [@iSazonov](https://github.com/iSazonov)!)
- Åtgärda ett fel i slut för ande av `native.exe --<tab>` flikar för att ringa till den interna slutföraren. (#3633) (Tack till [@powercode](https://github.com/powercode)!)
