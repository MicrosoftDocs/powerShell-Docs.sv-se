...
Din administratör för Windows PowerShell-webbåtkomsten bör ge dig en URL som är adressen till din organisations webbplats för Windows PowerShell-webbåtkomsten. Som standard är `https://<server_name>/pswa`den här webbplats adressen.

Innan du loggar in på Windows PowerShell-webbåtkomsten måste du kontrol lera att du har namnet eller IP-adressen för den fjärrdator som du vill hantera. Du måste vara en behörig användare på fjärrdatorn och den måste konfigureras för att tillåta fjärrhantering. Mer information om hur du konfigurerar din dator för att tillåta fjärrhantering finns i [Aktivera och använda fjärrkommandon i Windows PowerShell](/powershell/module/microsoft.powershell.core/enable-psremoting).

Den enklaste metoden att konfigurera datorn för att tillåta fjärrhantering är att köra `Enable-PSRemoting -force` cmdleten på datorn i en Windows PowerShell-session som har öppnats med utökade användar rättigheter (**Kör som administratör**).
...
1. På inloggnings sidan för Windows PowerShell-webbåtkomsten anger du ditt nätverks användar namn, lösen ord och namnet på den dator som du vill hantera (och där du är en auktoriserad användare). Om administratör för Windows PowerShell-webbåtkomsten har instruerat dig att använda en URI för en anpassad webbplats eller proxyserver i stället för ett dator namn väljer du **anslutnings-URI** i fältet **Anslutnings typ** och anger sedan URI.
   ...
   > - Om mål datorn är Gateway-servern och Gateway-servern finns i en arbets grupp måste du använda `<workgroup name>\<user_name>` det i användar namnet. Du kan använda `localhost` i fältet dator namn.

1. Avsnittet **valfria anslutnings inställningar** relaterar till auktoriserings kraven för den fjärrdator som du vill hantera. Mer information om de parametrar som motsvarar valfria anslutnings inställningar finns i hjälpen för cmdleten [Enter-PSSession](/powershell/module/microsoft.powershell.core/enter-pssession) .

   Normalt är de autentiseringsuppgifter som du använder för att skicka genom Windows PowerShell-webbåtkomst-gatewayen samma som identifieras av den fjärrdator som du vill hantera. Men om du vill använda andra autentiseringsuppgifter för att hantera den fjärrdator som du angav i steg 2 expanderar du avsnittet **valfria anslutnings inställningar** och anger alternativa autentiseringsuppgifter. Annars går du vidare till steg 6.

1. Om Windows PowerShell Web Access-administratören har skapat en anpassad sessionsnyckel för Windows PowerShell Web Access-användare skriver du namnet på sessionens konfigurations namn i fältet **konfigurations namn** . För ytterligare information om sessionskonfigurationer, se [about_Session_Configurations](/powershell/module/microsoft.powershell.core/about/about_session_configurations).
...
- Stänga av den klient enhet där webbläsaren körs, eller koppla från den från nätverket.

- Kör kommandot **exit** i webb konsolen. Det här kommandot fungerar inte om den sessionsinformation som du är ansluten till har kon figurer ATS för att stöda i läget [nolanguage](/dotnet/api/system.management.automation.pslanguagemode) eller i en begränsad körnings utrymme.
...
- Ändra färg på indatamängd.

  Indatatypen (både förgrunds och Background) kan inte ändras. Formatet på utdata-, varnings-, utförliga-och fel meddelanden kan ändras genom att köra ett skript.
...
### <a name="unsupported-shortcut-keys"></a>Kortkommandon som inte stöds
 ...
 Funktions nyckel   |                                                                                         Action
--------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Ctrl+C          | I Windows PowerShell-webbåtkomsten används **CTRL + C** av webbläsaren för att kopiera innehåll. Konsolen har en **Avbryt** -knapp och användarna kan också använda **CTRL + Q** för att avbryta kommandon.
Alt-blank steg, e, l | Bläddra igenom skärmens buffert
Alt + blank steg, e, f | Sök efter text i kommandobufferten
Alt + blank steg, e, k | Välj text som ska kopieras från skärmens buffert
Alt + blank steg, e, p | Klistra in innehållet i Urklipp i Windows PowerShell-konsolen
Alt + blank steg, c    | Stäng Windows PowerShell-konsolen
Ctrl + Break      | Tvinga Windows PowerShell-fönstret att stängas
Ctrl+Home       | Tar bort från början av den aktuella kommando raden
Ctrl + End        | Tar bort till slutet av kommando raden
F1              | Flytta markören ett steg till höger på kommando raden
F2              | Skapar ett nytt kommando genom att kopiera det senaste kommandot upp till det-tecknen som du skriver
F3              | Slutför kommando raden med innehåll från den sista kommando raden
F4              | Tar bort tecken från markörens position
F5              | Sök bakåt i kommando historiken. Om du vill komma åt kommandon i kommando historiken i Windows PowerShell-webbåtkomsten klickar du på rullnings knapparna **Historik** i den webbaserade konsolen.
F7              | Markera ett kommando från kommando historiken interaktivt
F8              | Genomsöknings historiken visar kommandon som matchar den aktuella texten
F9              | Köra ett speciellt numrerat kommando från historiken
Page Up         | Kör det första kommandot i historiken
Page Down       | Kör det sista kommandot i historiken
ALT + F7          | Rensa kommando historik listan

### <a name="limitations-of-the-web-based-console"></a>Begränsningar för den webbaserade-konsolen
...
- Dubbla hopp
  ...
  För att undvika fel med dubbla hopp bör administratören konfigurera CredSSP-autentisering i organisationens nätverks miljö. Mer information om hur du konfigurerar CredSSP-autentisering finns i [CredSSP för fjärr kommunikation med andra hopp](https://devblogs.microsoft.com/powershell/credssp-for-second-hop-remoting/) i PowerShell-bloggen. Du kan också ange explicita autentiseringsuppgifter när du vill hantera en andra fjärrdator. implicita autentiseringsuppgifter är troligen inte tillåtna för det andra hoppet.

- Tjänst

  Windows PowerShell-webbåtkomsten använder och har samma begränsningar som en fjärran sluten Windows PowerShell-session. Kommandon som direkt anropar API: er för Windows-konsolen, t. ex. för konsolbaserade redigerare eller textbaserade meny program, fungerar inte eftersom kommandona inte läser eller skriver till standar dina indata, utdata och fel pipes. Därför fungerar inte kommandon som startar en körbar fil, t. ex **. Notepad. exe**eller visar ett grafiskt användar `OpenGridView` gränssnitt `ogv`, till exempel eller. Din upplevelse påverkas av detta beteende. till dig visas det att Windows PowerShell-webbåtkomsten inte svarar på ditt kommando.

- Slut för ande flik

  TABB-slutförande fungerar inte i en sessionshantering med en begränsad körnings utrymme eller en som är i läget **nolanguage** . Administratörer kan konfigurera en session som stöder slut för ande av tabbar, men det rekommenderas inte av säkerhets skäl, eftersom den kan exponera följande information för obehöriga användare.

  - Interna fil Systems Sök vägar
  - Delade mappar på interna datorer
  - Variabler i körnings utrymme
  - Inlästa typer or.NET Framework-namnrymder
  - Miljövariabler

- **Nolanguage** -session, eller begränsat körnings utrymme

  Användare som är inloggade på en konfiguration av en **Nolanguage** -session eller en begränsad körnings utrymme i Windows PowerShell-webbåtkomsten kan inte köra **exit** -kommandot för att avsluta sessionen. För att logga ut ska användarna klicka på **Logga ut** på konsol sidan.

- Ansluta till flera mål datorer samtidigt.
...
- [Hejsan, Scripting Guy!](https://devblogs.microsoft.com/scripting/)
