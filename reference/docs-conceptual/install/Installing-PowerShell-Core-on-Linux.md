...
Ladda ned Debian-paketet `powershell_7.1.2-1.ubuntu.16.04_amd64.deb` från sidan [releases][] på Ubuntu-datorn.
...
Ladda ned Debian-paketet `powershell_7.1.2-1.ubuntu.18.04_amd64.deb` från sidan [releases][] på Ubuntu-datorn.
...
### <a name="installation-via-package-repository---ubuntu-2004"></a>Installation via paket lagring – Ubuntu 20,04
...
Ladda ned Debian-paketet `powershell_7.1.2-1.ubuntu.20.04_amd64.deb` från sidan [releases][] på Ubuntu-datorn.
...
### <a name="uninstallation---ubuntu-2004"></a>Avinstallation – Ubuntu 20,04
...
## <a name="ubuntu-1910-and-2010"></a>Ubuntu 19,10 och 20,10

Installationen stöds via `snapd` . Instruktioner finns i [snapin-paket][snap].

> [!NOTE]
> Ubuntu 19,10 är en [tillfällig version](https://www.ubuntu.com/about/release-cycle) som [stöds av communityn](../powershell-support-lifecycle.md).
...
Ladda ned Debian-paketet `powershell_7.1.2-1.debian.9_amd64.deb` från sidan [releases][] på Debian-datorn.
...
Hämta paketet tar. gz `powershell-7.1.2-linux-x64.tar.gz` från sidan [utgåvor][] till Debian-datorn.
...
Hämta paketet tar. gz `powershell-7.1.2-linux-alpine-x64.tar.gz` från sidan [utgåvor][] till Alpine Machine.
...
Med [CentOS 7][]laddar du ned rpm-paketet `powershell-7.1.2-1.rhel.7.x86_64.rpm` från sidan [utgåvor][] till CentOS-datorn.
...
Hämta RPM-paketet `powershell-7.1.2-1.rhel.7.x86_64.rpm` från sidan [versioner][] till Red Hat Enterprise Linux datorn.
...
Hämta RPM-paketet `powershell-7.1.2-1.rhel.7.x86_64.rpm` från sidan [releases][] på Fedora-datorn.
...
## <a name="support-for-arm-processors"></a>Stöd för ARM-processorer

PowerShell kan installeras på vissa Linux-distributioner. PowerShell är beroende av .NET-stöd för arm. PowerShell stöds i följande distributioner:

- Alpine Linux v 3.11 +-.NET stöder Arm64, men det finns inget installerbart paket för PowerShell för tillfället
- Raspbian – se installations anvisningarna nedan
- Debian v9 +-stöder Arm32 och Arm64 med hjälp av installations metoden för [binärt Arkiv](#binary-archives)
- Ubuntu 20,10, 20,04, 18,04, 16,04 – stöder Arm32 och Arm64 med installations metoden för [binärt Arkiv](#binary-archives)
...
I följande exempel visas stegen för att installera det binära arkivet x64. Du måste välja rätt binärt arkiv som matchar processor typen för din plattform.
...
- Användar profiler läses från `~/.config/powershell/profile.ps1`
- Standard profiler läses från `$PSHOME/profile.ps1`
- Användarens moduler läses från `~/.local/share/powershell/Modules`
