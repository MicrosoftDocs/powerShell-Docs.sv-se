...
Ladda ned PKG-paketet`powershell-lts-7.0.1-osx-x64.pkg`
...
PowerShell-binärfiler `tar.gz` tillhandahålls för MacOS-plattformen för att aktivera avancerade distributions scenarier.
...
* `$PSHOME` är `/usr/local/microsoft/powershell/7.0.1/`
...
Så att den standardinställda värdbaserade profilen finns på `Microsoft.PowerShell_profile.ps1` samma platser.
...
Så, `$PSHOME` är `/usr/local/microsoft/powershell/7.0.1/` och den symboliska länken placeras på `/usr/local/bin/pwsh` .
