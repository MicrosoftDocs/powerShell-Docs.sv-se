...
## <a name="why-do-dependency-conflicts-occur"></a><span data-ttu-id="4cc4f-112">Varför inträffar beroende konflikter?</span><span class="sxs-lookup"><span data-stu-id="4cc4f-112">Why do dependency conflicts occur?</span></span>
...
## <a name="powershell-and-net"></a><span data-ttu-id="4cc4f-141">PowerShell och .NET</span><span class="sxs-lookup"><span data-stu-id="4cc4f-141">PowerShell and .NET</span></span>
...
<span data-ttu-id="4cc4f-155">I den här artikeln kommer vi att se följande metoder:</span><span class="sxs-lookup"><span data-stu-id="4cc4f-155">In this article we'll refer to the following mechanisms:</span></span>
...
- <span data-ttu-id="4cc4f-205">Konflikten är ett direkt beroende av modulen och du styr versionen.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-205">Your conflict is with a direct dependency of your module and you control the version.</span></span>
...
<span data-ttu-id="4cc4f-219">Situationer där det inte går att fästa till en gemensam beroende version är:</span><span class="sxs-lookup"><span data-stu-id="4cc4f-219">Circumstances where pinning to a common dependency version won't work include:</span></span>
...
- <span data-ttu-id="4cc4f-232">När beroendet har gjort ändringar mellan versioner, eller när beroende kod är beroende av en funktion som annars inte är tillgänglig i den version som du omdirigerar till.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-232">When the dependency has made breaking changes between versions, or when dependent code relies on a functionality otherwise not available in the version you redirect to.</span></span>
...
- <span data-ttu-id="4cc4f-245">PowerShell-jobbet</span><span class="sxs-lookup"><span data-stu-id="4cc4f-245">The PowerShell job system</span></span>
...
### <a name="loading-through-net-core-assembly-load-contexts"></a><span data-ttu-id="4cc4f-270">Inläsning via .NET Core Assembly load-kontexter</span><span class="sxs-lookup"><span data-stu-id="4cc4f-270">Loading through .NET Core Assembly Load Contexts</span></span>
...
<span data-ttu-id="4cc4f-280">Cmdlet-implementeringen ser ut så här:</span><span class="sxs-lookup"><span data-stu-id="4cc4f-280">The cmdlet implementation looks like this:</span></span>
...
<span data-ttu-id="4cc4f-308">Cmdleten `AlcModule.Cmdlets.dll` ser ut så här:</span><span class="sxs-lookup"><span data-stu-id="4cc4f-308">The cmdlet in `AlcModule.Cmdlets.dll` looks like this:</span></span>
...
<span data-ttu-id="4cc4f-325">AlcModule. cmdlets. CSPROJ ser ut så här:</span><span class="sxs-lookup"><span data-stu-id="4cc4f-325">AlcModule.Cmdlets.csproj looks like:</span></span>
...
<span data-ttu-id="4cc4f-380">Dessa lösningar har ett gemensamt tema som de är ändringar i distributions konfigurationen för en miljö där du styr programmet och eventuellt hela datorn.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-380">These solutions have the common theme that they are changes to deployment configurations for an environment where you control the application and possibly the entire machine.</span></span> <span data-ttu-id="4cc4f-381">Dessa lösningar är riktade mot scenarier som webb servrar och andra program som distribueras till Server miljöer, där miljön är avsedd att vara värd för programmet och är kostnads fritt att konfigureras av distributions användaren.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-381">These solutions are oriented toward scenarios like web servers and other applications deployed to server environments, where the environment is intended to host the application and is free to be configured by the deploying user.</span></span> <span data-ttu-id="4cc4f-382">De brukar också vara mycket .NET Framework orienterade, vilket innebär att de inte fungerar med PowerShell 6 eller senare.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-382">They also tend to be very much .NET Framework oriented, meaning they do not work with PowerShell 6 or above.</span></span>

<span data-ttu-id="4cc4f-383">Om du vet att modulen endast används i Windows PowerShell 5,1-miljöer som du har total kontroll över, kan vissa av dessa alternativ vara.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-383">If you know that your module is only used in Windows PowerShell 5.1 environments that you have total control over, some of these may be options.</span></span> <span data-ttu-id="4cc4f-384">I allmänhet **bör moduler inte ändra globalt dator tillstånd som detta**.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-384">In general however, **modules should not modify global machine state like this**.</span></span> <span data-ttu-id="4cc4f-385">Den kan avbryta konfigurationer som orsakar problem i `powershell.exe` , andra moduler eller andra beroende program som orsakar att modulen kraschar på oväntade sätt.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-385">It can break configurations that cause problems in `powershell.exe`, other modules, or other dependent applications that cause your module to fail in unexpected ways.</span></span>

### <a name="static-binding-redirect-with-appconfig-to-force-using-the-same-dependency-version"></a><span data-ttu-id="4cc4f-386">Statisk bindning omdirigera med app.config för att framtvinga användning av samma beroende version</span><span class="sxs-lookup"><span data-stu-id="4cc4f-386">Static binding redirect with app.config to force using the same dependency version</span></span>

<span data-ttu-id="4cc4f-387">.NET Framework program kan dra nytta av en `app.config` fil för att konfigurera vissa av programmets beteenden i deklarativ form.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-387">.NET Framework applications can take advantage of an `app.config` file to configure some of the application's behaviors declaratively.</span></span> <span data-ttu-id="4cc4f-388">Det går att skriva en `app.config` post som konfigurerar sammansättnings bindning för att omdirigera sammansättnings inläsning till en viss version.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-388">It's possible to write an `app.config` entry that configures assembly binding to redirect assembly loading to a particular version.</span></span>

<span data-ttu-id="4cc4f-389">Två problem med detta för PowerShell är:</span><span class="sxs-lookup"><span data-stu-id="4cc4f-389">Two issues with this for PowerShell are:</span></span>

- <span data-ttu-id="4cc4f-390">.NET Core stöder inte `app.config` , så den här lösningen gäller endast för `powershell.exe` .</span><span class="sxs-lookup"><span data-stu-id="4cc4f-390">.NET Core does not support `app.config`, so this solution only applies to `powershell.exe`.</span></span>
- <span data-ttu-id="4cc4f-391">`powershell.exe` är ett delat program som finns i `System32` katalogen.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-391">`powershell.exe` is a shared application that lives in the `System32` directory.</span></span> <span data-ttu-id="4cc4f-392">Det är troligt att modulen inte kan ändra innehållet på många system.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-392">It's likely that your module won't be able to modify its contents on many systems.</span></span> <span data-ttu-id="4cc4f-393">Även om det kan ändra det kan det `app.config` bryta en befintlig konfiguration eller påverka inläsningen av andra moduler.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-393">Even if it can, modifying the `app.config` could break an existing configuration or affect the loading of other modules.</span></span>

### <a name="setting-codebase-with-appconfig"></a><span data-ttu-id="4cc4f-394">Ange `codebase` med app.config</span><span class="sxs-lookup"><span data-stu-id="4cc4f-394">Setting `codebase` with app.config</span></span>

<span data-ttu-id="4cc4f-395">Av samma skäl kommer att försöka konfigurera `codebase` inställningen i `app.config` inte att fungera i PowerShell-moduler.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-395">For the same reasons, trying to configure the `codebase` setting in `app.config` is not going to work in PowerShell modules.</span></span>

### <a name="installing-dependencies-to-the-global-assembly-cache-gac"></a><span data-ttu-id="4cc4f-396">Installera beroenden i GAC (Global Assembly Cache)</span><span class="sxs-lookup"><span data-stu-id="4cc4f-396">Installing dependencies to the Global Assembly Cache (GAC)</span></span>

<span data-ttu-id="4cc4f-397">Ett annat sätt att lösa beroende versions konflikter i .NET Framework är att installera beroenden till GAC, så att olika versioner kan läsas in sida vid sida från GAC.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-397">Another way to resolve dependency version conflicts in .NET Framework is to install dependencies to the GAC, so that different versions can be loaded side-by-side from the GAC.</span></span>

<span data-ttu-id="4cc4f-398">För PowerShell-moduler är problemen här:</span><span class="sxs-lookup"><span data-stu-id="4cc4f-398">Again, for PowerShell modules, the chief issues here are:</span></span>

- <span data-ttu-id="4cc4f-399">Den globala sammansättningscachen gäller endast .NET Framework, så det här är inte hjälp i PowerShell 6 och senare.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-399">The GAC only applies to .NET Framework, so this does not help in PowerShell 6 and above.</span></span>
- <span data-ttu-id="4cc4f-400">Att installera sammansättningar i GAC är en ändring av global dator status och kan orsaka sido effekter i andra program eller i andra moduler.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-400">Installing assemblies to the GAC is a modification of global machine state and may cause side-effects in other applications or to other modules.</span></span> <span data-ttu-id="4cc4f-401">Det kan också vara svårt att göra korrekt, även om modulen har de behörigheter som krävs.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-401">It may also be difficult to do correctly, even when your module has the required access privileges.</span></span> <span data-ttu-id="4cc4f-402">Att få IT-fel kan orsaka allvarliga, datorövergripande problem i andra .NET-program.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-402">Getting it wrong could cause serious, machine-wide issues in other .NET applications.</span></span>

## <a name="further-reading"></a><span data-ttu-id="4cc4f-403">Ytterligare läsning</span><span class="sxs-lookup"><span data-stu-id="4cc4f-403">Further reading</span></span>

<span data-ttu-id="4cc4f-404">Det finns mycket mer att läsa på .NET-sammansättning versions beroende konflikter.</span><span class="sxs-lookup"><span data-stu-id="4cc4f-404">There's plenty more to read on .NET assembly version dependency conflicts.</span></span> <span data-ttu-id="4cc4f-405">Här är några saker som är bra att säga:</span><span class="sxs-lookup"><span data-stu-id="4cc4f-405">Here are some nice jumping off points:</span></span>

- [<span data-ttu-id="4cc4f-406">.NET: sammansättningar i .NET</span><span class="sxs-lookup"><span data-stu-id="4cc4f-406">.NET: Assemblies in .NET</span></span>](/dotnet/standard/assembly/)
- [<span data-ttu-id="4cc4f-407">.NET Core: algoritmen för inläsning av hanterad sammansättning</span><span class="sxs-lookup"><span data-stu-id="4cc4f-407">.NET Core: The managed assembly loading algorithm</span></span>](/dotnet/core/dependency-loading/loading-managed)
- [<span data-ttu-id="4cc4f-408">.NET Core: förstå system. Runtime. Loader. AssemblyLoadContext</span><span class="sxs-lookup"><span data-stu-id="4cc4f-408">.NET Core: Understanding System.Runtime.Loader.AssemblyLoadContext</span></span>](/dotnet/core/dependency-loading/understanding-assemblyloadcontext)
- [<span data-ttu-id="4cc4f-409">.NET Core: diskussion om inläsnings lösningar sida vid sida</span><span class="sxs-lookup"><span data-stu-id="4cc4f-409">.NET Core: Discussion about side-by-side assembly loading solutions</span></span>](https://github.com/dotnet/runtime/issues/13471)
- [<span data-ttu-id="4cc4f-410">.NET Framework: omdirigering av sammansättnings versioner</span><span class="sxs-lookup"><span data-stu-id="4cc4f-410">.NET Framework: Redirecting assembly versions</span></span>](/dotnet/framework/configure-apps/redirect-assembly-versions)
- [<span data-ttu-id="4cc4f-411">.NET Framework: metod tips för sammansättnings inläsning</span><span class="sxs-lookup"><span data-stu-id="4cc4f-411">.NET Framework: Best practices for assembly loading</span></span>](/dotnet/framework/deployment/best-practices-for-assembly-loading)
- [<span data-ttu-id="4cc4f-412">.NET Framework: hur körningen hittar sammansättningar</span><span class="sxs-lookup"><span data-stu-id="4cc4f-412">.NET Framework: How the runtime locates assemblies</span></span>](/dotnet/framework/deployment/how-the-runtime-locates-assemblies)
- [<span data-ttu-id="4cc4f-413">.NET Framework: lös sammansättnings belastningar</span><span class="sxs-lookup"><span data-stu-id="4cc4f-413">.NET Framework: Resolve assembly loads</span></span>](/dotnet/standard/assembly/resolve-loads)
- [<span data-ttu-id="4cc4f-414">StackOverflow: sammansättnings bindning omdirigera, hur och varför?</span><span class="sxs-lookup"><span data-stu-id="4cc4f-414">StackOverflow: Assembly binding redirect, how and why?</span></span>](https://stackoverflow.com/questions/43365736/assembly-binding-redirect-how-and-why)
- [<span data-ttu-id="4cc4f-415">PowerShell: diskussion om att implementera AssemblyLoadContexts</span><span class="sxs-lookup"><span data-stu-id="4cc4f-415">PowerShell: Discussion about implementing AssemblyLoadContexts</span></span>](https://github.com/PowerShell/PowerShell/issues/11571)
- [<span data-ttu-id="4cc4f-416">PowerShell: `Assembly.LoadFile()` läses inte in i standard AssemblyLoadContext</span><span class="sxs-lookup"><span data-stu-id="4cc4f-416">PowerShell: `Assembly.LoadFile()` doesn't load into default AssemblyLoadContext</span></span>](https://github.com/PowerShell/PowerShell/issues/12052)
- [<span data-ttu-id="4cc4f-417">Rick-Strahl: när ett .NET-sammansättnings beroende hämtas läses in?</span><span class="sxs-lookup"><span data-stu-id="4cc4f-417">Rick Strahl: When does a .NET assembly dependency get loaded?</span></span>](https://weblog.west-wind.com/posts/2012/Nov/03/Back-to-Basics-When-does-a-NET-Assembly-Dependency-get-loaded)
- [<span data-ttu-id="4cc4f-418">Jon Skeet: Sammanfattning av versions hantering i .NET</span><span class="sxs-lookup"><span data-stu-id="4cc4f-418">Jon Skeet: Summary of versioning in .NET</span></span>](https://codeblog.jonskeet.uk/2019/06/30/versioning-limitations-in-net/)
- [<span data-ttu-id="4cc4f-419">Nate McMaster: djupgående att gå in i .NET Core-primitiver</span><span class="sxs-lookup"><span data-stu-id="4cc4f-419">Nate McMaster: Deep dive into .NET Core primitives</span></span>](https://natemcmaster.com/blog/2017/12/21/netcore-primitives/)
