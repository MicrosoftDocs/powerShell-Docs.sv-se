...
Stöd för PowerShell Core inkluderar inte produktmallar, om inte dessa moduler uttryckligen stöder PowerShell-kärnan. Exempel: med modulen `ActiveDirectory` som levereras som en del av Windows Server, är ett scenario som inte stöds.

Moduler som inte uttryckligen stöder PowerShell-kärnan kan dock vara kompatibla i vissa fall. Genom att installera modulen [`WindowsPSModulePath`][] kan du lägga till Windows PowerShell-`PSModulePath` till PowerShell-kärnan `PSModulePath`.

Först installerar du modulen `WindowsPSModulePath` från PowerShell-galleriet:
...
När du har installerat den här modulen kör du cmdleten `Add-WindowsPSModulePath` för att lägga till Windows PowerShell-`PSModulePath` till PowerShell-kärnan:
