...
Om du vill hitta information om parametrarna för ett skript använder du den fullständiga sökvägen till skript filen. Exempel:
...
Till exempel `Get-ChildItem` har cmdleten sökväg och exkludera parametrar. `Position`Inställningen för **sökväg** är **0**, vilket innebär att den är en positions parameter. `Position`Inställningen för **exclude** kallas .
