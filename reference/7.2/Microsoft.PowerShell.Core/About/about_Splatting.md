...
Använd eller om du vill ange  ett värde för parametern whatIf `$True` switch `$False` .
...
### <a name="example-1-reuse-splatted-parameters-in-different-commands"></a>Exempel 1: Återanvänd splatted-parametrar i olika kommandon
...
### <a name="example-2-forward-parameters-using-psboundparameters"></a>Exempel 2: vidarebefordra parametrar med $PSBoundParameters
...
### <a name="example-3-override-splatted-parameters-with-explicitly-defined-parameters"></a>Exempel 3: Åsidosätt splatted-parametrar med explicit definierade parametrar

I det här exemplet visas hur du åsidosätter en splatted-parameter med explicit definierade parametrar. Detta är användbart när du inte vill bygga en ny hash-hash eller ändra ett värde i den hash-tabellen som du använder för att splat.

`$commonParams`Variabeln lagrar parametrarna för att skapa virtuella datorer på `East US` platsen. `$allVms`Variabeln är en lista över virtuella datorer som ska skapas. Vi går igenom listan och använder `$commonParams` för att splat parametrarna för att skapa varje virtuell dator. Vi vill dock `myVM2` skapa i en annan region än de andra virtuella datorerna. I stället för att justera `$commonParams` hash-tabellen kan du uttryckligen definiera **plats** parametern i `New-AzVm` som ersätter värdet för `Location` nyckeln i `$commonParams` .
