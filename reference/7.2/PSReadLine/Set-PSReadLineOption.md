...
Du kan välja att bara ange en förgrunds text färg. Till exempel en ljus grön förgrunds text färg för **kommentarens** token: ``"Comment"="`e[92m"`` .
...
Två färg inställningar har lagts till för att stödja anpassning av `ListView` PSReadLine-2.2.0:

- **ListPredictionColor** – Ange färg för det inledande `>` tecknet och det efterföljande käll namnet, t. ex. `[History]` . Som standard används den `DarkYellow` som förgrunds färg.
- **ListPredictionSelectedColor** – Ange färg för att indikera att ett List objekt är markerat. Som standard används den `DarkBlack` som bakgrunds färg.
...
- **InlinePrediction**: färgen på den infogade vyn för det förutsägande förslaget.
...
Anger källan för PSReadLine för att få förutsägelse förslag.

Giltiga värden är:

- **Ingen** – inaktivera förutsägande IntelliSense-funktionen (standard).
-`**Historik** – aktivera den förutsägande IntelliSense-funktionen och Använd PSReadLine-historiken som den enda källan.
- **Plugin-program** – aktivera den förutsägande IntelliSense-funktionen och Använd plugin-programmet ( `CommandPrediction` ) som den enda källan. Det här värdet lades till i PSReadLine 2.2.0
- **HistoryAndPlugin** – aktivera funktionen för förutsägelse IntelliSense och Använd både historik och plugin-program som källor. Det här värdet lades till i PSReadLine 2.2.0
...
Anger formatet för visning av förutsägelse text. Standardvärdet är **InlineView**.

- **InlineView** – formatet som befintliga idag, på samma sätt som i fiskeri-och zsh. (standard)
- **ListView** – förslag återges i en nedrullningsbar listruta och användare kan välja att använda <kbd>uppilen</kbd> och <kbd>NEDPIL</kbd>.

Den här parametern har lagts till i PSReadLine 2.2.0
