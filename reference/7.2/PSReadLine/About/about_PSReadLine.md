...
PSReadLine 2,2 ger en kraftfull redigerings upplevelse för kommando tolken för PowerShell-konsolen. Den tillhandahåller:
...
PSReadLine 2.2.0 har lagt till två nya förutsägande IntelliSense-funktioner:

- Parametern **PredictionViewStyle** har lagts till för att det nya ska kunna väljas `ListView` .
- Anslutna PSReadLine till `CommandPrediction` API: erna introducerade i PS 7,1 för att tillåta en användare att importera en förutsägbar modul som kan återge förslag från en anpassad källa.

PSReadLine kräver PowerShell 3,0 eller senare. PSReadLine fungerar med standard konsol värden, Visual Studio Code och Window Terminal. Den fungerar inte i PowerShell ISE.

PSReadLine 2.2.0 levereras med PowerShell 7,2 och stöds i alla versioner av PowerShell som stöds. Den är tillgänglig för installation från PowerShell-galleriet.
Kör följande kommando för att installera PSReadLine-2.2.0 i en version av PowerShell som stöds.
...
- Vi kommando läge: `<Delete>` , `<x>` , `<d,l>` , `<d,Spacebar>`
...
Tar bort till slutet av Multiline-bufferten.

- Kommando läge för vi: `<d,G>`
...
Tar bort den aktuella logiska raden för en buffert med flera rader och aktiverar ångra.

- Kommando läge för vi: `<d,d>` , `<d,_>`

### <a name="deletepreviouslines"></a>DeletePreviousLines

Tar bort föregående begärda logiska rader och den aktuella logiska linjen i en buffert med flera rader.

- Kommando läge för vi: `<d,k>`

### <a name="deleterelativelines"></a>DeleteRelativeLines

Tar bort från buffertens början till den aktuella logiska linjen i en buffert med flera rader.

Som de flesta vi-kommandon `<d,g,g>` kan kommandot vara anpassningsprefix med ett numeriskt argument som anger ett absolut rad nummer, som tillsammans med det aktuella rad numret, utgör ett intervall med rader som ska tas bort.
Om inget värde anges är det numeriska argumentet standardvärdet 1, som refererar till den första logiska linjen i en buffert med flera rader.

Det faktiska antalet rader som ska tas bort från flera rader beräknas som skillnaden mellan det aktuella logiska rad numret och det angivna numeriska argumentet, som därför kan vara negativt. Därför är den _relativa_ delen av metod namnet.

- Kommando läge för vi: `<d,g,g>`

### <a name="deletenextlines"></a>DeleteNextLines

Tar bort den aktuella logiska raden och nästa begärda logiska rader i en buffert med flera rader.

- Kommando läge för vi: `<d,j>`

### <a name="deletelinetofirstchar"></a>DeleteLineToFirstChar

Tar bort från det första icke-tomma tecken i den aktuella logiska linjen i en buffert med flera rader.
...
- Kommando läge för vi: `<y,l>` , `<y,Spacebar>`
...
Flytta markören ett steg till vänster i redigerings läget i vi. Detta kan flytta markören till föregående rad med flera rader.
...
Flytta markören ett steg till höger i redigerings läget i vi. Detta kan flytta markören till nästa rad med flera rader.

- Vi infognings läge: `<RightArrow>`
- Vi kommando läge: `<RightArrow>` , `<Spacebar>` , `<l>`
...
- Emacs: `<Alt+=>`
- Vi infognings läge: `<Ctrl+Spacebar>`
- Kommando läge för vi: `<Ctrl+Spacebar>`
...
Dessa två funktioner används av `Get-PSReadLineKeyHandler` . Den första används för att hämta alla nyckel bindningar. Den andra används för att hämta vissa nyckel bindningar.
