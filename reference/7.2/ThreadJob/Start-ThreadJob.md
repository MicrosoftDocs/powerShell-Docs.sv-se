...
Den här parametern ger ett tråd säkert sätt att låta `Write-Host` utdata gå direkt till det skickade **PSHost** -objektet. Utan det `Write-Host` går utdata till data ström samlingen för jobb information och visas inte i en värd konsol förrän jobben har körts.
