...
`Get-WinEvent` visar händelse loggar och providers för händelse loggar. Tryck på <kbd>CTRL</kbd>C om du vill avbryta kommandot + <kbd></kbd>. Du kan hämta händelser från valda loggar eller från loggar som skapats av valda händelse leverantörer. Du kan också kombinera händelser från flera källor i ett enda kommando.
