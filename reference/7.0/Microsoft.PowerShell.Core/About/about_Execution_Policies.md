...
På datorer som inte är Windows-datorer är standard körnings principen **obegränsad** och kan inte ändras. `Set-ExecutionPolicy`Cmdleten är tillgänglig, men PowerShell visar ett konsol meddelande som inte stöds. Medan `Get-ExecutionPolicy` returnerar **obegränsade** på plattformar som inte är Windows-plattformar, matchar **Bypass** beteendet i själva verket eftersom dessa plattformar inte implementerar Windows-säkerhetszoner.

## <a name="powershell-execution-policies"></a>Körnings principer för PowerShell

Tvång av dessa principer sker endast på Windows-plattformar. PowerShell-körnings principerna är följande:
...
- Standard körnings principen för datorer som inte är Windows-datorer och inte kan ändras.
...
Du kan använda parametern **ExecutionPolicy** för **pwsh.exe** om du vill ange en körnings princip för en ny PowerShell-session. Principen påverkar endast den aktuella sessionen och de underordnade sessionerna.

Ange körnings principen för en ny session genom att starta PowerShell på kommando raden, till exempel **cmd.exe** eller från PowerShell, och sedan använda parametern **ExecutionPolicy** för **pwsh.exe** för att ange körnings principen.
...
- Körnings princip: process (eller `pwsh.exe -ExecutionPolicy` )
...
När PowerShell 6 körs på Windows Server Core eller Windows Nano Server under vissa förhållanden, kan körnings principer inte utföras med följande fel:
...
[Hjälp om Pwsh-konsolen](about_pwsh.md)
