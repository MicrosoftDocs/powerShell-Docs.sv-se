...
Dessa operatörer använder `$?` `$LASTEXITCODE` variablerna och för att avgöra om en pipeline misslyckades. På så sätt kan du använda dem med inbyggda kommandon och inte bara med cmdlets eller functions. Ett exempel:
...
Precis som de flesta andra operatorer i PowerShell, `&&` och `||` är också *kvar att associera* , vilket innebär att de är grupperade från vänster. Ett exempel:
