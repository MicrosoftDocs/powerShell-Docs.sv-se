...
### Exempel 5: tur och retur i en enda element mat ris

Det här kommandot visar ett exempel där `-NoEnumerate` växeln används för att runda av en JSON-matris med ett enda element.
...
JSON-strängen innehåller en matris med ett enda element. Utan växeln konverterar JSON till en PSObject och konverterar sedan tillbaka den med `ConvertTo-Json` kommandot till ett enda heltal.
...
Om du anger den här parametern skickas matriser som ett enda objekt i stället för att varje element skickas separat. Detta garanterar att JSON kan vara Round-utlöst via `ConvertTo-Json` .
