...
`Find-Module`Använder parametern **Name** för att ange **PowerShellGet** -modulen. Den nyaste versionen av modulen laddas som standard ned från lagrings platsen. Objektet skickas ned pipelinen till `Install-Module` cmdleten. `Install-Module` installerar modulen för alla användare i `$env:ProgramFiles\PowerShell\Modules` .
...
`Install-Module` laddar ned och installerar den senaste versionen av **PowerShellGet** i den aktuella användarens katalog `$home\Documents\PowerShell\Modules` .
...
Som standard installeras moduler för omfånget för **allusers** i `$env:ProgramFiles\PowerShell\Modules` . Standard förhindrar förvirring när du installerar PowerShell-resurser för Desired State Configuration (DSC).
