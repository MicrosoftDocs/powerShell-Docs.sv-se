...
`Get-Process`Cmdleten hämtar processerna på en lokal dator.
...
Det här exemplet innehåller ett `Format-Table` kommando som lägger till egenskapen **MachineName** i `Get-Process` standardutdata-visningen.
...
Detta kommando använder parametern **FileVersionInfo** för att hämta versions information för den pwsh.exe-fil som är huvud-till-modul för PowerShell-processen.
...
Det här kommandot visar hur du hittar ägaren till en process.
I Windows kräver parametern **IncludeUserName** utökade användar rättigheter (kör som administratör).
