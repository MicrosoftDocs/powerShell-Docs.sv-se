...
`Clear-ItemProperty`Cmdleten rensar värdet för en egenskap, men den tar inte bort egenskapen.
Du kan använda den här cmdleten för att ta bort data från ett register värde.

## EXEMPEL

### Exempel 1: Rensa värdet för register nyckeln

Det här kommandot rensar data i registervärdet "alternativ" i under nyckeln "mitt" i `HKEY_LOCAL_MACHINE\Software\MyCompany` .
...
Anger att den här cmdleten tar bort egenskaper från objekt som annars inte kan kommas åt av användaren. Implementeringen varierar från providern till providern.
...
Anger namnet på den egenskap som ska rensas, till exempel namnet på ett register värde.
...
Anger sökvägen till den egenskap som rensas.
Jokertecken är tillåtna.
...
### Ingen eller system. Management. Automation. PSCustomObject

När du använder parametern **Passthru** `Clear-ItemProperty` genererar ett **PSCustomObject** -objekt som representerar egenskapen rensat objekt. Annars genererar denna cmdlet inga utdata.

## ANTECKNINGAR

- Du kan använda `Clear-ItemProperty` för att ta bort data i register värden utan att ta bort värdet.
  Om värdets datatyp är Binary eller DWORD, ställer du in värdet på noll om du rensar data.
  Annars är värdet tomt.
- `Clear-ItemProperty`Cmdleten är utformad för att fungera med data som exponeras av vilken provider som helst. Om du vill visa en lista över tillgängliga providers i din session skriver du `Get-PSProvider` . Mer information finns i [about_Providers](../Microsoft.PowerShell.Core/About/about_Providers.md).
