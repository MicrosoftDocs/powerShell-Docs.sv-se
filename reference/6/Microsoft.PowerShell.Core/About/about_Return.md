...
I PowerShell returneras resultatet av varje instruktion som utdata, även om det inte finns någon instruktion som innehåller `return` nyckelordet. Språk som C eller C \# returnerar bara de värden eller värden som anges av `return` nyckelordet.

> [!NOTE]
> Från och med PowerShell 5,0 har PowerShell använt språk för att definiera klasser med hjälp av formell syntax.  I kontexten för en PowerShell-klass är inget utdata från en metod, förutom vad du anger en `return` instruktion. Du kan läsa mer om PowerShell-klasser i [about_Classes](about_Classes.md).
