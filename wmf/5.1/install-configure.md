...
2. Högerklicka på ZIP-filen och välj ”extrahera alla...”. Zip innehåller 2 filer: en MSU- och installera WMF5.1.PS1 skriptfilen.
När du har packat ZIP-filen, kan du kopiera innehållet till en dator som kör Windows 7 eller Windows Server 2008 R2.

3. När innehållet för ZIP-filen, öppnar du PowerShell som administratör och navigera till mappen som innehåller innehållet i ZIP-filen.
...
Windows PowerShell önskad tillstånd Configuration (DSC) är beroende av WinRM.
WinRM är inte aktiverad som standard i Windows Server 2008 R2 och Windows 7.
Kör `Set-WSManQuickConfig`, utökade sessionen att aktivera WinRM i en Windows PowerShell.
...
> [!NOTE]
> Installera WMF 5.1 kräver en omstart. Med den `/quiet` alternativet startas datorn utan varning.
> Använd den `/norestart` alternativet för att undvika att startas om. WMF 5.1 kommer inte installeras förrän du har startats om.
