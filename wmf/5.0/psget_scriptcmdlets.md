...
ms.locfileid: "55687848"
---
# <a name="powershellget-cmdlets-for-script-management"></a><span data-ttu-id="d6a9a-102">PowerShellGet-cmdletar för skripthantering</span><span class="sxs-lookup"><span data-stu-id="d6a9a-102">PowerShellGet Cmdlets for Script Management</span></span>

## <a name="find-script-cmdlet"></a><span data-ttu-id="d6a9a-103">Find-Script-cmdlet</span><span class="sxs-lookup"><span data-stu-id="d6a9a-103">Find-Script cmdlet</span></span>
<span data-ttu-id="d6a9a-104">Find-Script cmdlet kan du identifiera skriptfiler med andra sökvillkor som namn, tagg, filter, kommandonamnet, versionsintervallet, exakt version, alla versioner, inklusive dess beroenden och från specifika eller alla registrerade databaser.</span><span class="sxs-lookup"><span data-stu-id="d6a9a-104">Find-Script cmdlet lets you to discover the script files with different search criteria like name, tag, filter, command name, version range, exact version, all versions, including its dependencies and from specific or all registered repositories.</span></span>

<span data-ttu-id="d6a9a-105">Exempel på användning:</span><span class="sxs-lookup"><span data-stu-id="d6a9a-105">Example usage:</span></span>
```powershell
...
## <a name="save-script-cmdlet"></a><span data-ttu-id="d6a9a-106">Cmdleten Save-Script</span><span class="sxs-lookup"><span data-stu-id="d6a9a-106">Save-Script cmdlet</span></span>
<span data-ttu-id="d6a9a-107">Save-Script cmdlet kan du granska skriptfilen genom att spara den till en angiven plats.</span><span class="sxs-lookup"><span data-stu-id="d6a9a-107">Save-Script cmdlet lets you to review the script file by saving it to a specified location.</span></span>
```powershell
...
## <a name="install-script-and-get-installedscript-cmdlets"></a><span data-ttu-id="d6a9a-108">Installationsskriptet och Get-InstalledScript cmdlet: ar</span><span class="sxs-lookup"><span data-stu-id="d6a9a-108">Install-Script and Get-InstalledScript cmdlets</span></span>
<span data-ttu-id="d6a9a-109">Installationsskriptet cmdlet kan du installera en specifik skriptfil tillsammans med dess beroenden i det specificerade omfånget.</span><span class="sxs-lookup"><span data-stu-id="d6a9a-109">Install-Script cmdlet lets you to install a specific script file along with its dependencies to the specified scope.</span></span> <span data-ttu-id="d6a9a-110">Som standard installeras skript AllUsers omfattning.</span><span class="sxs-lookup"><span data-stu-id="d6a9a-110">By default, scripts are installed to the AllUsers scope.</span></span> <span data-ttu-id="d6a9a-111">Get-InstalledScript cmdlet kan du hämta en lista över filer som har installerats med hjälp av cmdleten Install-Script.</span><span class="sxs-lookup"><span data-stu-id="d6a9a-111">Get-InstalledScript cmdlet lets you to get the list of script files which were installed using Install-Script cmdlet.</span></span>

<span data-ttu-id="d6a9a-112">Använd Obs: För att tillåta hantering och lokalisering av skript när de installeras installationsskriptet skapar en standardmappen för lagring av skript på $home\Documents\WindowsPowerShell\Scripts, och lägga till mappen i PATH-miljö.</span><span class="sxs-lookup"><span data-stu-id="d6a9a-112">Use note: To allow management and locating of scripts once they are installed, Install-script will create a default folder for storing scripts at $home\Documents\WindowsPowerShell\Scripts, and add that folder to your PATH environment.</span></span> <span data-ttu-id="d6a9a-113">Om du ändrar sökvägen är ett problem kan du använda Save-Script i stället för Install-Script.</span><span class="sxs-lookup"><span data-stu-id="d6a9a-113">If modifying the path is a concern, use Save-Script instead of Install-Script.</span></span> <span data-ttu-id="d6a9a-114">Get-InstalledScripts och Avinstallationsskriptet fungerar bara med placeras i systemet med Install-Script-skript.</span><span class="sxs-lookup"><span data-stu-id="d6a9a-114">Get-InstalledScripts and Uninstall-Script can only work with scripts placed on the system using Install-Script.</span></span>
```powershell
...
<span data-ttu-id="d6a9a-115">Du kan också använda Get-Command – namn &lt;InstalledScriptFileName&gt; att hämta den.</span><span class="sxs-lookup"><span data-stu-id="d6a9a-115">You can also use Get-Command –Name &lt;InstalledScriptFileName&gt; to get it.</span></span> <span data-ttu-id="d6a9a-116">Installationsplats för två läggs till i miljövariabeln PATH vid första användning av ett angivet omfång.</span><span class="sxs-lookup"><span data-stu-id="d6a9a-116">Two install locations are added to the PATH environment variable on first use of a specified scope.</span></span>
```powershell
...
## <a name="update-script-cmdlet"></a><span data-ttu-id="d6a9a-117">Cmdlet: en Update-Script</span><span class="sxs-lookup"><span data-stu-id="d6a9a-117">Update-Script cmdlet</span></span>
<span data-ttu-id="d6a9a-118">Cmdlet: en Update-skript kan du en uppdatering av skriptfiler som installerades med hjälp av cmdleten Install-Script direkt på plats.</span><span class="sxs-lookup"><span data-stu-id="d6a9a-118">Update-Script cmdlet lets you to do in-place update of the script files which were installed using Install-Script cmdlet.</span></span>
```powershell
...
## <a name="uninstall-script-cmdlet"></a><span data-ttu-id="d6a9a-119">Avinstallera skript-cmdlet</span><span class="sxs-lookup"><span data-stu-id="d6a9a-119">Uninstall-Script cmdlet</span></span>
```powershell
...
## <a name="new-scriptfileinfo-and-test-scriptfileinfo-cmdlets"></a><span data-ttu-id="d6a9a-120">Cmdletar för nya ScriptFileInfo och Test-ScriptFileInfo</span><span class="sxs-lookup"><span data-stu-id="d6a9a-120">New-ScriptFileInfo and Test-ScriptFileInfo cmdlets</span></span>
<span data-ttu-id="d6a9a-121">Ny ScriptFileInfo cmdlet kan du skapa en ny skriptfil med metadata som Version, Guid, författare och beskrivning, osv. Test-ScriptFileInfo cmdlet kan du för att verifiera och hämta skriptfilens metadata.</span><span class="sxs-lookup"><span data-stu-id="d6a9a-121">New-ScriptFileInfo cmdlet lets you to create a new script file with metadata like Version, Guid, Author, and Description, etc. Test-ScriptFileInfo cmdlet lets you to validate and get the script file metadata.</span></span>
```powershell
...
## <a name="update-scriptfileinfo-cmdlet"></a><span data-ttu-id="d6a9a-122">Cmdlet: en Update-ScriptFileInfo</span><span class="sxs-lookup"><span data-stu-id="d6a9a-122">Update-ScriptFileInfo cmdlet</span></span>
<span data-ttu-id="d6a9a-123">Uppdatera ScriptFileInfo cmdlet kan du uppdatera den befintliga skriptfilens metadata.</span><span class="sxs-lookup"><span data-stu-id="d6a9a-123">Update-ScriptFileInfo cmdlet lets you to update the existing script file metadata.</span></span>
```powershell
...
## <a name="register-psrepository-and-set-psrepository-cmdlets-with-script-sharing-support"></a><span data-ttu-id="d6a9a-124">Register-PSRepository och Set-PSRepository cmdlets med skriptet delning support</span><span class="sxs-lookup"><span data-stu-id="d6a9a-124">Register-PSRepository and Set-PSRepository cmdlets with script sharing support</span></span>
<span data-ttu-id="d6a9a-125">Använda Register-PSRepository/Set-PSRepository cmdletar för att lägga till den **ScriptSourceLocation** och **ScriptPublishLocation** till PSRepository.</span><span class="sxs-lookup"><span data-stu-id="d6a9a-125">Use Register-PSRepository/Set-PSRepository cmdlets to add the **ScriptSourceLocation** and **ScriptPublishLocation** to the PSRepository.</span></span>
```powershell
...
## <a name="publish-script-cmdlet"></a><span data-ttu-id="d6a9a-126">Publicera skript-cmdlet</span><span class="sxs-lookup"><span data-stu-id="d6a9a-126">Publish-Script cmdlet</span></span>
<span data-ttu-id="d6a9a-127">Publicera skriptet cmdlet kan du publicera skriptfilen med giltig metadata som Version, Guid, författare och beskrivning, osv.</span><span class="sxs-lookup"><span data-stu-id="d6a9a-127">Publish-Script cmdlet lets you to publish your script file with valid metadata like Version, Guid, Author, and Description, etc.</span></span>
```powershell
